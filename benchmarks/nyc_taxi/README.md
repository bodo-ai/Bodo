...
4. For reading and writing to S3, you will need to set up an IAM role with sufficient permissions. You can do this using the AWS console by going to __IAM__ > __Roles__ > __Create Role__, then selecting EC2 as the service and `AmazonS3FullAccess` as the permission policy. Name the role "dask-benchmark". Or, you can run this script, which uses the AWS SDK.
```
# TODO fillme
```
