# Original queries derived from https://github.com/coiled/benchmarks/blob/13ebb9c72b1941c90b602e3aaea82ac18fafcddc/tests/tpch/dask_queries.py
...
def _load_dataset(dataset_path, fs, table_name, ext=".parquet"):
    return dd.read_parquet(dataset_path / f"{table_name}{ext}", filesystem=fs).rename(
        columns=lambda x: x.lower()
    )


def query_01(dataset_path, fs, scale, ext=".parquet"):
    VAR1 = datetime(1998, 9, 2)
    lineitem_ds = _load_dataset(dataset_path, fs, "lineitem", ext)
...
def query_02(dataset_path, fs, scale, ext=".parquet"):
    var1 = 15
    var2 = "BRASS"
    var3 = "EUROPE"

    region_ds = _load_dataset(dataset_path, fs, "region", ext)
    nation_filtered = _load_dataset(dataset_path, fs, "nation", ext)
    supplier_filtered = _load_dataset(dataset_path, fs, "supplier", ext)
    part_filtered = _load_dataset(dataset_path, fs, "part", ext)
    partsupp_filtered = _load_dataset(dataset_path, fs, "partsupp", ext)
...
def query_03(dataset_path, fs, scale, ext=".parquet"):
    var1 = datetime.strptime("1995-03-15", "%Y-%m-%d")
    var2 = "BUILDING"

    lineitem_ds = _load_dataset(dataset_path, fs, "lineitem", ext)
    orders_ds = _load_dataset(dataset_path, fs, "orders", ext)
    cutomer_ds = _load_dataset(dataset_path, fs, "customer", ext)
...
def query_04(dataset_path, fs, scale, ext=".parquet"):
    date1 = datetime.strptime("1993-10-01", "%Y-%m-%d")
    date2 = datetime.strptime("1993-07-01", "%Y-%m-%d")

    line_item_ds = _load_dataset(dataset_path, fs, "lineitem", ext)
    orders_ds = _load_dataset(dataset_path, fs, "orders", ext)
...
def query_05(dataset_path, fs, scale, ext=".parquet"):
    date1 = datetime.strptime("1994-01-01", "%Y-%m-%d")
    date2 = datetime.strptime("1995-01-01", "%Y-%m-%d")

    region_ds = _load_dataset(dataset_path, fs, "region", ext)
    nation_ds = _load_dataset(dataset_path, fs, "nation", ext)
    customer_ds = _load_dataset(dataset_path, fs, "customer", ext)
    line_item_ds = _load_dataset(dataset_path, fs, "lineitem", ext)
    orders_ds = _load_dataset(dataset_path, fs, "orders", ext)
    supplier_ds = _load_dataset(dataset_path, fs, "supplier", ext)
...
def query_06(dataset_path, fs, scale, ext=".parquet"):
    date1 = datetime.strptime("1994-01-01", "%Y-%m-%d")
    date2 = datetime.strptime("1995-01-01", "%Y-%m-%d")
    var3 = 24
...
def query_07(dataset_path, fs, scale, ext=".parquet"):
...
def query_08(dataset_path, fs, scale, ext=".parquet"):
    var1 = datetime.strptime("1995-01-01", "%Y-%m-%d")
    var2 = datetime.strptime("1997-01-01", "%Y-%m-%d")

    supplier = _load_dataset(dataset_path, fs, "supplier", ext)
    lineitem = _load_dataset(dataset_path, fs, "lineitem", ext)
    orders = _load_dataset(dataset_path, fs, "orders", ext)
    customer = _load_dataset(dataset_path, fs, "customer", ext)
    nation = _load_dataset(dataset_path, fs, "nation", ext)
    region = _load_dataset(dataset_path, fs, "region", ext)
    part = _load_dataset(dataset_path, fs, "part", ext)
...
def query_09(dataset_path, fs, scale, ext=".parquet"):
    """
    select
        nation,
        o_year,
    ...
    part = _load_dataset(dataset_path, fs, "part", ext)
    partsupp = _load_dataset(dataset_path, fs, "partsupp", ext)
    supplier = _load_dataset(dataset_path, fs, "supplier", ext)
    lineitem = _load_dataset(dataset_path, fs, "lineitem", ext)
    orders = _load_dataset(dataset_path, fs, "orders", ext)
    nation = _load_dataset(dataset_path, fs, "nation", ext)
...
def query_10(dataset_path, fs, scale, ext=".parquet"):
    """
    select
        c_custkey,
        c_name,
...
def query_11(dataset_path, fs, scale, ext=".parquet"):
    """
    select
        ps_partkey,
...
def query_12(dataset_path, fs, scale, ext=".parquet"):
    """
    select
        l_shipmode,
        sum(case
...
def query_13(dataset_path, fs, scale, ext=".parquet"):
    """
    select
...
def query_14(dataset_path, fs, scale, ext=".parquet"):
    """
    select
...
def query_15(dataset_path, fs, scale, ext=".parquet"):
    """
    DDL:
...
def query_16(dataset_path, fs, scale, ext=".parquet"):
    """
    select
        p_brand,
        p_type,
...
def query_17(dataset_path, fs, scale, ext=".parquet"):
    """
    select
...
def query_18(dataset_path, fs, scale, ext=".parquet"):
    """
    select
        c_name,
        c_custkey,
...
def query_19(dataset_path, fs, scale, ext=".parquet"):
    """
    select
        round(sum(l_extendedprice* (1 - l_discount)), 2) as revenue
    from
...
def query_20(dataset_path, fs, scale, ext=".parquet"):
    """
    select
        s_name,
        s_address
...
def query_21(dataset_path, fs, scale, ext=".parquet"):
    """
    select
        s_name,
        count(*) as numwait
...
def query_22(dataset_path, fs, scale, ext=".parquet"):
    """
    select
        cntrycode,
        count(*) as numcust,
