...
def snowflake_to_python_re(pattern, flags):
    ...
    pattern = bodosql.kernels.regexp_array_kernels.posix_to_re(pattern)
    flags = bodosql.kernels.regexp_array_kernels.make_flag_bit_vector(flags)
...
def test_regexp_count(regexp_strings, regexp_like_count_args):
    def impl(arr, pattern, position, flags):
        return pd.Series(bodosql.kernels.regexp_count(arr, pattern, position, flags))
...
def test_regexp_like(regexp_strings, regexp_like_count_args):
    def impl(arr, pattern, flags):
        return pd.Series(bodosql.kernels.regexp_like(arr, pattern, flags))
...
def test_regexp_empty_pattern(args):
    ...
    def impl1(source):
        return bodosql.kernels.regexp_like(source, "", "")

    def impl2(source):
        return bodosql.kernels.regexp_count(source, "", 1, "")

    def impl3(source):
        return bodosql.kernels.regexp_replace(source, "", "*", 1, 0, "")

    def impl4(source):
        return bodosql.kernels.regexp_substr(source, "", 1, 1, "", 1)

    def impl5(source):
        return bodosql.kernels.regexp_instr(source, "", 1, 1, 0, "", 1)
...
def test_regexp_posix_unsupported(args):
    ...
    def impl(arr, pattern, position, occurrence, flags, group):
        return bodosql.kernels.regexp_substr(
            arr, pattern, position, occurrence, flags, group
        )
...
def test_option_regexp(func):
    def impl1(optFlag):
        ...
        pattern = r"(the)\s(\S+)"
        flag = "ie"
        return bodosql.kernels.regexp_like(source, pattern, flag)
    ...
    def impl2(optFlag):
        ...
        pattern = r"(the) (\w+)"
        flag = "ie"
        return bodosql.kernels.regexp_count(source, pattern, pos, flag)
    ...
    def impl4(optFlag):
        ...
        return bodosql.kernels.regexp_substr(source, pattern, pos, occur, flag, group)
    ...
    def impl5(optFlag):
        ...
        flag = "ie"
        group = 2 if optFlag else None
        return bodosql.kernels.regexp_instr(
...
def test_regexp_error(func, args, in_series):
    ...
    def impl1(arr, pattern, position, flags):
        return bodosql.kernels.regexp_count(arr, pattern, position, flags)
