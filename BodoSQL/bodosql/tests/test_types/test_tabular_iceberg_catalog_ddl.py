...
def check_view_exists(polaris_connection, view_name) -> bool:
...
def check_table_exists(polaris_connection, table_name) -> bool:
    # These should only be run from rank 0, but we can't mark them as
    # run_rank0 since they are used in other run_rank0 functions
    assert bodo.get_rank() == 0
    uri, warehouse, credential = polaris_connection
...
@contextmanager
def view_helper(polaris_connection, view_name, create=True):
    if create:

        @run_rank0
        def create_view():
            get_spark(polaris_connection).sql(
                f"CREATE OR REPLACE VIEW {view_name} AS SELECT 0 AS A"
            )
            assert check_view_exists(polaris_connection, view_name)
    ...
    try:
        ...
        @run_rank0
        def destroy_view():
            get_spark(polaris_connection).sql(f"DROP VIEW IF EXISTS {view_name}")
            assert not check_view_exists(polaris_connection, view_name)

        destroy_view()


@pytest_mark_one_rank
def test_create_view(tabular_catalog, polaris_connection, memory_leak_check):
    ...
    # execute_ddl Version
    with view_helper(polaris_connection, view_name, create=False):
...
def schema_helper(polaris_connection, schema_name, create=True):
    if create:
        ...
        @run_rank0
        def create_schema():
            get_spark(polaris_connection).sql(f"CREATE SCHEMA {schema_name}")
    ...
    try:
        ...
        @run_rank0
        def destroy_schema():
            get_spark(polaris_connection).sql(f"DROP SCHEMA IF EXISTS {schema_name}")
...
@pytest_mark_one_rank
def test_create_view_validates(tabular_catalog, polaris_connection, memory_leak_check):
...
@contextmanager
def view_helper_nontrivialview(bc, polaris_connection, view_name, create=True):
...
def test_iceberg_drop_table_purge_sql(
    purge, tabular_catalog, polaris_connection, memory_leak_check
):
    """
...
def test_iceberg_describe_view_basic(
    describe_keyword, tabular_catalog, polaris_connection, memory_leak_check
):
    """
    ...
    with view_helper_nontrivialview(bc, polaris_connection, view_name, create=True):
        # execute_ddl Version
...
# Helper functions
def verify_table_created(table_name, polaris_connection, bc):
    assert check_table_exists(polaris_connection, table_name)
...
def drop_test_table(table_name, polaris_connection):
    # drop created table
    get_spark(polaris_connection).sql(f"DROP TABLE IF EXISTS {table_name}")
    get_spark(polaris_connection).sql(f"DROP TABLE IF EXISTS {table_name}_renamed")


# Verify view was created.
def verify_view_created(view_name, polaris_connection, bc):
    assert check_view_exists(polaris_connection, view_name)
    x = bc.sql(f"SELECT * FROM {view_name}")
    x = bodo.allgatherv(x)
    assert "A" in x
...
def drop_test_view(view_name, polaris_connection):
    # drop created view
    get_spark(polaris_connection).sql(f"DROP VIEW IF EXISTS {view_name}")
    get_spark(polaris_connection).sql(f"DROP VIEW IF EXISTS {view_name}_renamed")


@pytest_mark_one_rank
def test_alter_view(tabular_catalog, polaris_connection, memory_leak_check):
    """Tests that Bodo can rename a view via ALTER VIEW using a Tabular catalog."""
    bc = bodosql.BodoSQLContext(catalog=tabular_catalog)
    view_name = gen_unique_id("TEST_VIEW").upper()
    try:
        create_test_view(view_name, bc)
        verify_view_created(view_name, polaris_connection, bc)
        ...
        verify_view_created(f"{view_name}_renamed", polaris_connection, bc)

    finally:
        # drop created views
        drop_test_view(view_name, polaris_connection)


@pytest_mark_one_rank
def test_alter_view_rename_ifexists(
    tabular_catalog, polaris_connection, memory_leak_check
):
...
def test_alter_view_rename_to_existing(
    ...
    try:
        ...
        # Verify view was created.
        verify_view_created(view_name, polaris_connection, bc)
        verify_view_created(f"{view_name}_B", polaris_connection, bc)
        ...
        # drop created view
        drop_test_view(view_name, polaris_connection)
        drop_test_view(f"{view_name}_B", polaris_connection)


@pytest_mark_one_rank
def test_alter_table_on_view(tabular_catalog, polaris_connection, memory_leak_check):
...
@pytest_mark_one_rank
def test_alter_view_on_table(tabular_catalog, polaris_connection, memory_leak_check):
    ...
    try:
        ...
        verify_table_created(table_name, polaris_connection, bc)
        ...
        # drop created table
        drop_test_table(table_name, polaris_connection)
...
def test_iceberg_drop_view(
    if_exists, tabular_catalog, polaris_connection, memory_leak_check
):
    ...
    # execute_ddl Version
    with view_helper(polaris_connection, view_name, create=True):
        bodo_output = bc.execute_ddl(query_drop_view)
        _test_equal_guard(bodo_output, py_output)
        assert not run_rank0(check_view_exists)(polaris_connection, view_name)
