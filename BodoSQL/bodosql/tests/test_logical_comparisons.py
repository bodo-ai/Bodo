"""
Test correctness of SQL queries containing orderby on BodoSQL
"""

import pytest

from bodosql.tests.utils import check_query

pytestmark = pytest.mark.bodosql_cpp


def test_logical_or_columns(bodosql_boolean_types, memory_leak_check):
    """insures that logical logical or works  correctly for columns of boolean values"""
    query1 = "SELECT A or B from table1"
    check_query(
        query1,
        bodosql_boolean_types,
        None,
        check_dtype=False,
        check_names=False,
        use_duckdb=True,
    )


def test_logical_and_columns(bodosql_boolean_types, memory_leak_check):
    """insures that logical and works correctly for columns of boolean values"""
    query2 = "SELECT A and B from table1"
    check_query(
        query2,
        bodosql_boolean_types,
        None,
        check_dtype=False,
        check_names=False,
        use_duckdb=True,
    )


@pytest.mark.slow
def test_logical_and_scalars(bodosql_boolean_types, memory_leak_check):
    """insures that logical and works correctly for scalar boolean values"""
    query1 = "SELECT (CASE WHEN A and B THEN 0 ELSE 1 end) from table1"
    check_query(
        query1,
        bodosql_boolean_types,
        None,
        check_dtype=False,
        check_names=False,
        use_duckdb=True,
    )


@pytest.mark.slow
def test_logical_or_scalars(bodosql_boolean_types, memory_leak_check):
    """insures that logical or works correctly for scalar boolean values"""
    query2 = "SELECT (CASE WHEN A or B THEN 0 ELSE 1 end) from table1"
    check_query(
        query2,
        bodosql_boolean_types,
        None,
        check_dtype=False,
        check_names=False,
        use_duckdb=True,
    )


@pytest.fixture(params=["TRUE", "FALSE", "UNKNOWN"])
def is_clause_literal_values(request):
    return request.param


def test_logical_is_columns(
    bodosql_boolean_types, is_clause_literal_values, memory_leak_check
):
    """ensures that logical IS NOT works correctly for columns of boolean values"""
    query = f"SELECT A IS {is_clause_literal_values} from table1"
    check_query(
        query,
        bodosql_boolean_types,
        None,
        check_dtype=False,
        check_names=False,
        use_duckdb=True,
    )


@pytest.mark.slow
def test_logical_is_scalars(
    bodosql_boolean_types, is_clause_literal_values, memory_leak_check
):
    """ensures that logical is works correctly for scalar boolean values"""
    query = f"SELECT (CASE WHEN A is {is_clause_literal_values} THEN 0 ELSE 1 end) from table1"
    check_query(
        query,
        bodosql_boolean_types,
        None,
        check_dtype=False,
        check_names=False,
        use_duckdb=True,
    )


def test_is_logical_not_columns(
    bodosql_boolean_types, is_clause_literal_values, memory_leak_check
):
    """ensures that logical IS works correctly for columns of boolean values"""
    query = f"SELECT A IS NOT {is_clause_literal_values} from table1"
    check_query(
        query,
        bodosql_boolean_types,
        None,
        check_dtype=False,
        check_names=False,
        use_duckdb=True,
    )


@pytest.mark.slow
def test_logical_is_not_scalars(
    bodosql_boolean_types, is_clause_literal_values, memory_leak_check
):
    """ensures that logical is works correctly for scalar boolean values"""
    query = f"SELECT (CASE WHEN A is not {is_clause_literal_values} THEN 0 ELSE 1 end) from table1"
    check_query(
        query,
        bodosql_boolean_types,
        None,
        check_dtype=False,
        check_names=False,
        use_duckdb=True,
    )
