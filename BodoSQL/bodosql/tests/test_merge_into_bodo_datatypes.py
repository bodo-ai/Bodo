...
bodo_datatype_cols = {
    ...
    "str_col": pd.Series([str(i) for i in range(10)], dtype="string"),
    "bool_col": pd.Series([bool(i % 2) for i in range(10)], dtype=bool),
    "ts_col": pd.Series(
        [pd.Timestamp("2020-01-01") + pd.Timedelta(days=i) for i in range(10)],
    ),
    "non_ascii_col": pd.Series([str(i) + "é" for i in range(10)], dtype="string"),
...
def test_merge_into_bodo_datatypes_as_values(
    ...
    query = (
        ...
        f"  INSERT ({', '.join(bodo_datatype_cols.keys())}) "
        f"    values ({', '.join(['s.' + key for key in bodo_datatype_cols.keys()])})"
    )
...
def test_merge_into_bodo_datatypes_as_expr(
    col_name, iceberg_database, iceberg_table_conn, memory_leak_check
):
    """
    ...
    spark = get_spark()
    db_schema, warehouse_loc = iceberg_database
    table_name = "target_table_merge_into_bodo_datatypes_as_exprs"
