...
def generate_and_write_library():
    ...
    library_fns_info = (
        ...
        + [
            ...
            # numeric Fn's
            ("ceil", generate_standard_python_fn_call("np.ceil"), 1),
            ("floor", generate_standard_python_fn_call("np.floor"), 1),
            # TODO: need a wrapper around mod, mysql behavior is different
            # from np.mod on negative values
            ("mod", generate_standard_python_fn_call("np.mod"), 2),
            ("sign", generate_standard_python_fn_call("np.sign"), 1),
            ("round", generate_standard_python_fn_call("round"), 2),
            ("abs", generate_standard_python_fn_call("abs"), 1),
            ("log2", generate_standard_python_fn_call("np.log2"), 1),
            ("log10", generate_standard_python_fn_call("np.log10"), 1),
...
def generate_and_write_library_tests():
    ...
    library_tests_info = [
        ...
        (
            "ceil",
            library_fn_from_name("ceil"),
            [["1.0123"], ["-1.123"], ["123.012312"]],
            [2, -1, 124],
            False,
        ),
        (
            "floor",
            library_fn_from_name("floor"),
            [["1.0123"], ["-1.123"], ["123.012312"]],
            [1, -2, 123],
            False,
        ),
        (
            "mod",
            library_fn_from_name("mod"),
            [[18, 3], [1, 0], [13, 4], [-5.67, 3.123]],
            [0, 0, 1, 0.576],
            True,
        ),
        (
            "sign",
            library_fn_from_name("sign"),
            [[0], [12], [1.21321], [-123], [-23.2]],
            [0, 1, 1, -1, -1],
            False,
        ),
        (
            "round",
            library_fn_from_name("round"),
            [[-13.1234, 0], [-13.1234, -1], [123.123456, 2]],
            [-13, -10, 123.12],
            True,
        ),
        (
            "abs",
            library_fn_from_name("abs"),
            [[12], [-13], [-17.893]],
            [12, 13, 17.893],
            True,
        ),
        (
            "log2",
            library_fn_from_name("log2"),
            [[1], [8], [12], [23.23]],
            [0, 3, 3.58496250072, 4.53791724903],
            True,
        ),
        (
            "log10",
            library_fn_from_name("log10"),
            [[1], [1000], [24], [17.98]],
            [0, 3, 1.38021124171, 1.2547896874],
            True,
        ),
        (
            "ln",
            library_fn_from_name("ln"),
            [[1], [100], [13.2]],
            [0, 4.60517018599, 2.58021682959],
            True,
        ),
        (
            "exp",
            library_fn_from_name("exp"),
            [[0], [2], [2.45]],
            [1, 7.38905609893, 11.5883467192],
            True,
        ),
        (
            "pow",
            library_fn_from_name("pow"),
            [[0, 0], [2.23, 0], [2.45, -0.32], [-2, 3]],
            [1, 1, 0.75070075561, -8],
            True,
        ),
        (
