package com.bodosql.calcite.catalog.repository;
...
public class DatabaseRepository {
  ...
  private static SessionFactory getSessionFactory() {
    ...
    configuration = configuration.setProperty("hibernate.connection.url", "jdbc:h2:mem:bodosql");
  ...
  public CatalogDatabaseImpl getDatabase(Long dbId) {
    ...
    try {
      CatalogDatabaseImpl db =
          (CatalogDatabaseImpl) sessionObj.load(CatalogDatabaseImpl.class, dbId);
  ...
  */
  ...
  public CatalogDatabaseImpl getDatabase(String dbName) {
    ...
    try {
      ...
      CatalogDatabaseImpl db =
          (CatalogDatabaseImpl) criteria.add(Restrictions.eq("name", dbName)).uniqueResult();
