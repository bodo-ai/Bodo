...
internal fun List<OrderKey>.fieldList(fn: (List<Expr>) -> Expr = Expr::List): Expr =
    fn(this.map { Expr.StringLiteral(it.field) })

internal fun List<OrderKey>.ascendingList(fn: (List<Expr>) -> Expr = Expr::List): Expr =
