 ...
 */
abstract class AbstractJoinExtractOverRule protected constructor(config: Config) :
    ...
    private class RexOverExtractor(var condition: RexNode) {
        /**
         ...
         */
        fun push(builder: RelBuilder, rel: RelNode) {
            builder.push(rel)
            ...
            condition.accept(object : RexVisitorImpl<Unit>(true) {
        ...
        fun project(builder: RelBuilder, rel: RelNode, exprs: List<RexOver>) {
         ...
         */
        fun conditionRewrite(condition: RexNode, nodes: List<RexOver>): RexNode {
            return condition.accept(object : RexShuttle() {
                override fun visitInputRef(inputRef: RexInputRef?): RexNode {
                    return when (val index = inputRef!!.index) {
                ...
                override fun visitOver(over: RexOver?): RexNode {
                    return when (val index = nodes.indexOf(over!!)) {
                        -1 -> over
         ...
         * removes them.
         */
        fun join(builder: RelBuilder, joinType: JoinRelType) {
