...
/**
 * Planner rule that pushes an {@link Aggregate} past a {@link Join}.
 *
 * @see CoreRules#AGGREGATE_JOIN_TRANSPOSE
 * @see CoreRules#AGGREGATE_JOIN_TRANSPOSE_EXTENDED
 *     <p>Bodo version is due to a correctness issue involving the 0 row case. See
 *     https://bodo.atlassian.net/browse/BSE-1954
 */
...
public class BodoAggregateJoinTransposeRule extends RelRule<BodoAggregateJoinTransposeRule.Config>
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    for (int s = 0; s < 2; s++) {
      ...
      if (unique) {
        ...
        // BODO CHANGE: disallow pushing an aggregation without a group into
        // if the original had at least 1 group
        ...
        for (Ord<AggregateCall> aggCall : Ord.zip(aggregate.getAggCallList())) {
          ...
          if (call1 != null) {
            side.split.put(
                aggCall.i, belowAggregateKey.cardinality() + belowAggCallRegistry.register(call1));
          }
        }
        side.newInput =
            relBuilder
  ...
  /**
   * Computes the closure of a set of columns according to a given list of constraints. Each 'x = y'
   * constraint causes bit y to be set if bit x is set, and vice versa.
   */
  private static ImmutableBitSet keyColumns(
      ImmutableBitSet aggregateColumns, ImmutableList<RexNode> predicates) {
  ...
  /** Creates a {@link SqlSplittableAggFunction.Registry} that is a view of a list. */
  ...
  public interface Config extends RelRule.Config {
    Config DEFAULT =
        ImmutableBodoAggregateJoinTransposeRule.Config.of()
    ...
    @Override
    default BodoAggregateJoinTransposeRule toRule() {
      return new BodoAggregateJoinTransposeRule(this);
    }
    ...
    /** Defines an operand tree for the given classes, and also sets {@link #isAllowFunctions()}. */
