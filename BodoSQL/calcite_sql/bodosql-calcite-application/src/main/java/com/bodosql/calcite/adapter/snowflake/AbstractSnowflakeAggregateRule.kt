...
@Value.Enclosing
abstract class AbstractSnowflakeAggregateRule protected constructor(
    config: Config,
    ...
    companion object {
        ...
        private fun transformsToDistinct(aggCallList: List<AggregateCall>): Boolean =
            aggCallList.isEmpty() || aggCallList.all { x -> x.aggregation.kind == SqlKind.LITERAL_AGG }
        ...
        private fun isPushableDistinct(aggregate: Aggregate): Boolean =
            transformsToDistinct(aggregate.aggCallList) && Aggregate.isSimple(aggregate)
