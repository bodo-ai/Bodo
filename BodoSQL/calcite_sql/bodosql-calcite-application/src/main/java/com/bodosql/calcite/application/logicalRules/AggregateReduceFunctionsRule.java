...
public class AggregateReduceFunctionsRule extends RelRule<AggregateReduceFunctionsRule.Config>
  ...
  private RexNode reduceAgg(
    ...
    if (canReduceAggCallByGrouping(oldAggRel, oldCall)) {
      ...
      switch (kind) {
        ...
        case CORR:
          // BODO CHANGE: this logic is specific to Bodo since Calcite does not have a CORR rule.
          // replace original CORR(x, y) with
          //   COVAR_POP(x, y) / (STDDEV_POP(x) * STDDEV_POP(y))
          return reduceCorrelation(oldAggRel, oldCall, newCalls, aggCallMapping, inputExprs);
        case REGR_SXX:
      ...
      return rexBuilder.addAggCall(
          oldCall, nGroups, newCalls, aggCallMapping, oldAggRel.getInput()::fieldIsNullable);
    }
  }
  ...
  private static RexNode reduceAvg(
    ...
    RexNode numeratorRef =
        rexBuilder.addAggCall(
            sumCall, nGroups, newCalls, aggCallMapping, oldAggRel.getInput()::fieldIsNullable);
    final RexNode denominatorRef =
        rexBuilder.addAggCall(
            countCall, nGroups, newCalls, aggCallMapping, oldAggRel.getInput()::fieldIsNullable);

    final RelDataTypeFactory typeFactory = oldAggRel.getCluster().getTypeFactory();
    // BODO CHANGE: remove casting of the numerator to avoid decimal typing issues
  ...
  private static RexNode reduceSum(
    ...
    RexNode sumZeroRef =
        rexBuilder.addAggCall(
            sumZeroCall, nGroups, newCalls, aggCallMapping, oldAggRel.getInput()::fieldIsNullable);
    ...
    return rexBuilder.makeCall(
        ...
        rexBuilder.makeCall(
            SqlStdOperatorTable.EQUALS, countRef, rexBuilder.makeExactLiteral(BigDecimal.ZERO)),
  ...
  private static RexNode reduceStddev(
    ...
    // BODO CHANGE: do not cast the inputs to the output type.
    final RexNode argRef = inputExprs.get(argOrdinal);

    final RexNode argSquared = rexBuilder.makeCall(SqlStdOperatorTable.MULTIPLY, argRef, argRef);
    final int argSquaredOrdinal = lookupOrAdd(inputExprs, argSquared);

    // BODO CHANGE: ensure the filter arg is passed here as well.
    ...
    final RexNode sumArg =
        rexBuilder.addAggCall(
            ...
            aggCallMapping,
            oldAggRel.getInput()::fieldIsNullable);
    // BODO CHANGE: remove casting to avoid decimal typing issues
    ...
    if (sqrt) {
      // BODO CHANGE: ensure the inputs to POWER have been pre-casted to double
      final RexNode numerator = rexBuilder.makeCast(oldCall.getType(), div);
      final RexNode half = rexBuilder.makeLiteral(0.5, oldCall.getType());
      result = rexBuilder.makeCall(SqlStdOperatorTable.POWER, numerator, half);
    } else {
      result = div;
    }
  ...
  private static RexNode reduceCovariance(
    ...
    final RexNode argX = inputExprs.get(argXOrdinal);
    final RexNode argY = inputExprs.get(argYOrdinal);
    final RexNode argXAndYNotNullFilter =
        rexBuilder.makeCall(
            SqlStdOperatorTable.AND,
            rexBuilder.makeCall(SqlStdOperatorTable.IS_NOT_NULL, argX),
    ...
    // BODO CHANGE: replace REGR_COUNT(X, Y) with COUNT(X*Y)
    final AggregateCall countAggCall =
        createAggregateCallWithBinding(
            cluster.getTypeFactory(),
            SqlStdOperatorTable.COUNT,
            argXY.getType(),
            ...
            argXYOrdinal,
            argXAndYNotNullFilterOrdinal);
    boolean hasNullableCount = argXY.getType().isNullable();
    final RexNode countArg =
        cluster
            .getRexBuilder()
            .addAggCall(
                ...
                newCalls,
                aggCallMapping,
                idx -> hasNullableCount);

    final RexNode result = divide(biased, rexBuilder, sumXY, sumXSumY, countArg);
    return rexBuilder.makeCast(oldCall.getType(), result);
  }

  /** Bodo function: decomposes CORR into component aggregations. */
  private static RexNode reduceCorrelation(
      Aggregate oldAggRel,
      AggregateCall oldCall,
      List<AggregateCall> newCalls,
      Map<AggregateCall, RexNode> aggCallMapping,
      List<RexNode> inputExprs) {
    // corr(x, y) ==>
    //     covar_pop(x * y) / NULLIF(stddev_pop(x) * stddev_pop(y), 0)
    // But only on rows where x & y are both non-null
    final RelOptCluster cluster = oldAggRel.getCluster();
    final RexBuilder rexBuilder = cluster.getRexBuilder();
    final RelDataTypeFactory typeFactory = cluster.getTypeFactory();
    assert oldCall.getArgList().size() == 2 : oldCall.getArgList();
    ...
    final Aggregate.AggCallBinding covarBinding =
        new Aggregate.AggCallBinding(
            typeFactory,
            SqlStdOperatorTable.COVAR_POP,
            ImmutableList.of(),
            ImmutableList.of(argX.getType(), argY.getType()),
            oldAggRel.getGroupCount(),
            false);
    final AggregateCall covarCall =
        AggregateCall.create(
            SqlStdOperatorTable.COVAR_POP,
            oldCall.isDistinct(),
            oldCall.isApproximate(),
            ...
            oldCall.distinctKeys,
            oldCall.collation,
            SqlStdOperatorTable.COVAR_POP.inferReturnType(covarBinding),
            null);
    final RexNode covarArg =
        reduceCovariance(oldAggRel, covarCall, true, newCalls, aggCallMapping, inputExprs);

    final AggregateCall stdXCall =
        createAggregateCallWithBinding(
            cluster.getTypeFactory(),
    ...
    final RexNode stdXArg =
        reduceStddev(oldAggRel, stdXCall, true, true, newCalls, aggCallMapping, inputExprs);
    final AggregateCall stdYCall =
        createAggregateCallWithBinding(
            cluster.getTypeFactory(),
            SqlStdOperatorTable.STDDEV_POP,
            argY.getType(),
    ...
    final RexNode stdYArg =
        reduceStddev(oldAggRel, stdYCall, true, true, newCalls, aggCallMapping, inputExprs);

    // Convert the denominator to NULL if it is zero.
    final RexNode product = rexBuilder.makeCall(SqlStdOperatorTable.MULTIPLY, stdXArg, stdYArg);
    final RexNode nullProduct =
        rexBuilder.makeCall(
            CondOperatorTable.NULLIF, product, rexBuilder.makeLiteral(0, oldCallType));
    final RexNode result = rexBuilder.makeCall(SqlStdOperatorTable.DIVIDE, covarArg, nullProduct);
    return rexBuilder.makeCast(oldCall.getType(), result);
  }

  private static RexNode divide(
      boolean biased, RexBuilder rexBuilder, RexNode sumXY, RexNode sumXSumY, RexNode countArg) {
  ...
  /**
   * Does a shallow clone of oldAggRel and updates aggCalls. Could be refactored into Aggregate and
  ...
  public interface Config extends RelRule.Config {
    Config DEFAULT =
        ImmutableAggregateReduceFunctionsRule.Config.of().withOperandFor(LogicalAggregate.class);

    // BODO CHANGE: remove SUM and REGR functions, and added CORR
