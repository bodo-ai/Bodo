...
@Value.Enclosing
abstract class AbstractSnowflakeFilterRule protected constructor(
    config: Config,
    ...
    companion object {
            ...
            return condition.accept(
                object : RexVisitorImpl<Boolean?>(true) {
                    ...
                    override fun visitCall(call: RexCall?): Boolean {
                        ...
                        return if (call != null &&
                            (
                                SUPPORTED_CALLS.contains(call.kind) ||
                                    isSupportedOtherFunction(call) ||
                                    isSupportedCast(
                                        call,
                                    ) ||
                                    isSupportedLike(call) ||
                                    isSupportedDateAdd(call)
                            )
                        ) {
         ...
         */
        @JvmStatic
        fun isPartiallyPushableFilter(filter: Filter): Boolean = FilterUtils.isPartiallyPushableFilter(filter, ::isPushableCondition)

        /**
         * Determine if all of a filter is pushable.
         */
        @JvmStatic
        fun isFullyPushableFilter(filter: Filter): Boolean = FilterUtils.isFullyPushableFilter(filter, ::isPushableCondition)
    }
}
