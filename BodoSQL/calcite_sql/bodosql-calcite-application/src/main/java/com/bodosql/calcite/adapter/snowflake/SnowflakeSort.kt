...
class SnowflakeSort private constructor(
...
) : Sort(cluster, traitSet.replace(SnowflakeRel.CONVENTION), input, collation, offset, fetch),
    SnowflakeRel {
    override fun copy(
        traitSet: RelTraitSet,
        newInput: RelNode,
        ...
        offset: RexNode?,
        fetch: RexNode?,
    ): Sort = SnowflakeSort(cluster, traitSet, newInput, newCollation, offset, fetch, catalogTable)
    ...
    companion object {
        ...
        ): SnowflakeSort = SnowflakeSort(cluster, traitSet, input, collation, offset, fetch, catalogTable)
    }
}
