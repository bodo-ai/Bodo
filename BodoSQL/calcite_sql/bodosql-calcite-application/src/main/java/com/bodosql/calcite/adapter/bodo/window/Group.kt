...
internal class Group(
     ...
     */
    private fun partitionBy(arrayRexToBodoTranslator: ArrayRexToBodoTranslator): List<PartitionKey> =
        window.partitionKeys
            .mapIndexed { i, n -> PartitionKey("GRPBY_COL_$i", arrayRexToBodoTranslator.apply(n)) }

    /**
     * Computes the ordering column names along with the accessor expression for constructing the column.
     */
    private fun orderBy(arrayRexToBodoTranslator: ArrayRexToBodoTranslator): List<OrderKey> =
        window.orderKeys
            .mapIndexed { i, collation ->
                val expr = arrayRexToBodoTranslator.apply(collation.left)
                val (name, asc) =
     ...
     */
    private fun aggregateInputs(arrayRexToBodoTranslator: ArrayRexToBodoTranslator): List<Field> =
        localRefs.mapIndexed { i, n -> Field("AGG_OP_$i", arrayRexToBodoTranslator.apply(n), n.type) }
}
