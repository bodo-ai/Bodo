...
public class SubQueryRemoveRule extends RelRule<SubQueryRemoveRule.Config>
  ...
  protected SubQueryRemoveRule(SubQueryRemoveRule.Config config) {
    super(config);
    Objects.requireNonNull(config.matchHandler());
    Objects.requireNonNull(config.skippedIds());
  }
  ...
  protected RexNode apply(
      RexSubQuery e,
      Set<CorrelationId> initialVariablesSet,
      ...
      int subQueryIndex,
      boolean isJoin) {
    // Bodo Change: Prune any variables mark as skipped.
    Set<CorrelationId> variablesSet = new HashSet();
    for (CorrelationId variable : initialVariablesSet) {
      if (!config.skippedIds().contains(variable)) {
        variablesSet.add(variable);
      }
    }
    // Bodo Change: Correlated sub-queries inside join are extremely broken. They only produce
    // a correlated result with the right table even if the correlated variable is associated
    // with the left table. As a result, we disable them.
    if (isJoin && !variablesSet.isEmpty()) {
      return e;
    }
    switch (e.getKind()) {
      case SCALAR_QUERY:
  ...
  private static void matchProject(SubQueryRemoveRule rule, RelOptRuleCall call) {
    ...
    final RexNode target = rule.apply(e, variablesSet, logic, builder, 1, fieldCount, 0, false);
  ...
  public interface Config extends RelRule.Config {
    ...
    SubQueryRemoveRule.Config withMatchHandler(MatchHandler<SubQueryRemoveRule> matchHandler);

    // Bodo Change: Add an interface to skip IDs.
    SubQueryRemoveRule.Config withSkippedIds(Iterable<? extends CorrelationId> elements);

    Set<CorrelationId> skippedIds();
  }

  // New Function: verifies that no sub query remains anywhere in the plan
  public static void verifyNoSubQueryRemaining(RelNode rel) {
    NoSubQueryRelVisitor relVisitor = new NoSubQueryRelVisitor();
    relVisitor.go(rel);
    if (relVisitor.foundSubQuery) {
      throw new RuntimeException(
          "Found subquery in plan that could not be expanded:\n" + RelOptUtil.toString(rel));
    }
  }

  protected static class NoSubQueryRelVisitor extends RelVisitor {
    public boolean foundSubQuery = false;
    final NoSubQueryRexVisitor rexVisitor = new NoSubQueryRexVisitor(true);
  ...
  protected static class NoSubQueryRexVisitor extends RexVisitorImpl<Void> {
    public boolean foundSubQuery = false;

    protected NoSubQueryRexVisitor(boolean deep) {
      super(deep);
    }
