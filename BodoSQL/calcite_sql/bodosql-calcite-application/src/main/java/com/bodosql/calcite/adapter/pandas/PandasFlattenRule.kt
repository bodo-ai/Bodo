...
class PandasFlattenRule private constructor(config: Config) : ConverterRule(config) {
    ...
    override fun convert(rel: RelNode): RelNode {
        ...
        return PandasFlatten.create(rel.cluster, convert(input, input.traitSet.replace(convention)), flatten.call, flatten.callType, flatten.usedColOutputs, flatten.repeatColumns)
    }
}
