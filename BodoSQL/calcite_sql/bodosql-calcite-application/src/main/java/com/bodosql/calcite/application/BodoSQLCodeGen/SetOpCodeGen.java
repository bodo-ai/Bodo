...
public class SetOpCodeGen {
  ...
  public static Expr generateUnionCode(
    ...
    unionBuilder.append("bodo.hiframes.pd_dataframe_ext.union_dataframes((");
    for (int i = 0; i < childExprs.size(); i++) {
      unionBuilder.append(childExprs.get(i).emit()).append(", ");
    }
    unionBuilder.append("), ");
    if (isAll) {
    ...
    unionBuilder.append(", ").append(globalVarName.emit()).append(")");
    return new Expr.Raw(unionBuilder.toString());
  }
  ...
  public static List<Op> generateIntersectCode(
    ...
    if (isAll) {
      ...
      // Generate temp vars
      final Variable lhsDfCnt = new Variable(pdVisitorClass.genDfVar());
      final Variable rhsDfCnt = new Variable(pdVisitorClass.genDfVar());

      Op lhsCumcountCode = generateCumcountDf(lhsDfCnt, lhsExpr, lhsColNames, pdVisitorClass);
      Op rhsCumcountCode = generateCumcountDf(rhsDfCnt, rhsExpr, rhsColNames, pdVisitorClass);
  ...
  public static List<Op> generateExceptCode(
    ...
    if (isAll) {
      ...
      final Variable lhsDfCnt = new Variable(pdVisitorClass.genDfVar());
      final Variable rhsDfCnt = new Variable(pdVisitorClass.genDfVar());
      final Variable rhsDfTmp = new Variable(pdVisitorClass.genDfVar());
  ...
  private static Op generateCumcountDf(
    ...
    //   tableOnes = logical_table_to_table(get_dataframe_all_data(inputDfVar), (colOnes,),
    ...
    //   tableCnt = logical_table_to_table(get_dataframe_all_data(inputDfVar), (colCnt,),
    //                                     MetaType(...dummyColIdxs...), inputDfVar.shape[1])
    //   dfCnt = init_dataframe((tableCnt,), pd.RangeIndex(0, len(inputDfVar), 1),
    //                          ColNamesMetaType(...colNames..., "__bodo_dummy__"))

    // Generate temp vars
    final Variable colOnes = new Variable(pdVisitorClass.genSeriesVar());
    final Variable tableOnes = new Variable(pdVisitorClass.genTableVar());
    final Variable dfOnes = new Variable(pdVisitorClass.genDfVar());
    final Variable colCnt = new Variable(pdVisitorClass.genSeriesVar());
    final Variable tableCnt = new Variable(pdVisitorClass.genTableVar());
