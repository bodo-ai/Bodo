...
import com.bodosql.calcite.catalog.connection.CatalogServiceImpl;
 ...
 *
 * This is the class which stores the state of the application. It has a single {@link #schema} for
 * now. It also is what has access to the {@link #catalogService} which is used for persistence as
 * well as the {@link #relationalAlgebraGenerator} for converting sql to a {@see
...
public class ApplicationContext {
  /**
   * Handles all ddl queries. Persists this information to whatever source has been configured for
   * hibernate.
   */
  private CatalogServiceImpl catalogService;
  // assuming just one database for now
  /** Stores the schema with all the tables and their definitions. */
  private BodoSqlSchema schema;
  ...
  /**
   * Initializes the application context by calling the private constructor if necessary before any
   * operations occur.
   */
  ...
  /**
   * Refreshes the schema after ddl has occurred. The application context needs to reload every time
