...
/**
 * Class which holds main function. Listens in on a TCP socket for protocol buffer requests and then
...
public class CalciteApplication {
  static final Logger LOGGER = LoggerFactory.getLogger(CalciteApplication.class);

  public static void executeUpdate()
      throws NamingException, SQLException, LiquibaseException, InstantiationException,
    ...
    try {
      ...
      dataSource.setUsername("bodosql");
      dataSource.setPassword("bodosql");
      dataSource.setUrl("jdbc:h2:mem:bodosql");
      ...
      Liquibase liquibase =
          new Liquibase(
              changeLogFile, new CompositeResourceAccessor(clFO, fsFO, threadClFO), database);

      // @SuppressWarnings("unchecked")
      // StringTokenizer initParameters = new StringTokenizer(""); //
  ...
  private static CalciteApplicationOptions parseArguments(String[] arguments) {
    ...
    final Option orchestratorPort =
        Option.builder("oport")
            .required(false)
            .longOpt("orch_port")
            .hasArg()
            .argName("INTEGER")
    ...
    try {
      ...
      final Integer tcpPort =
          Integer.valueOf(
              commandLine.getOptionValue(tcpPortOption.getLongOpt(), tcpPortDefaultValue));
      CalciteApplicationOptions calciteApplicationOptions = null;
      final String orchIp =
          commandLine.getOptionValue(orchestratorIp.getLongOpt(), orchestratorIpDefaultValue);
      final Integer orchPort =
          Integer.valueOf(
              commandLine.getOptionValue(
                  orchestratorPort.getLongOpt(), orchestratorPortDefaultValue));
