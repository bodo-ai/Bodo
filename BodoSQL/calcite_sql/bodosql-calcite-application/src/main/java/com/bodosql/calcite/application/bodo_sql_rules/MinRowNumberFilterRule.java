 ...
 * then also replaced with a constant 1.
 *
...
public class MinRowNumberFilterRule extends RelRule<MinRowNumberFilterRule.Config>
  ...
  /** Determines if the given LogicalFilter is exactly col = 1. */
  public static boolean equalsOneFilter(Filter filter) {
    ...
    if (cond.getKind() == SqlKind.EQUALS) {
      ...
      if (operands.size() == 2) {
        if (operands.get(0) instanceof RexInputRef && operands.get(1) instanceof RexLiteral) {
          ...
          return literal.getValue().toString().equals("1");
  ...
  public static int getWindowColNum(final Filter filter) {
  ...
  public static boolean isColumnValidRowNumber(final Project project, final int windowColNum) {
  ...
  public void buildNewFilter(
      RelBuilder builder, final Project origProject, final int windowColNum) {
    // Fetch the original Window column
    List<RexNode> colProjects = origProject.getProjects();
    RexOver overNode = (RexOver) colProjects.get(windowColNum);
  ...
  public void buildNewProject(
    ...
    for (int i = 0; i < oldProjects.size(); i++) {
      if (i == windowColNum) {
        // Replace the OverNode with a RexLiteral 1, the output
        // of the filter. We need to do a cast to ensure the types match.
        newProjects.add(i, builder.cast(builder.literal(1), SqlTypeName.BIGINT));
      } else {
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    final int windowColNum = getWindowColNum(origFilter);
    // Verify that we have a matching row number
    if (!isColumnValidRowNumber(origProject, windowColNum)) {
      return;
    }
    ...
    buildNewFilter(builder, origProject, windowColNum);
    buildNewProject(builder, origProject, windowColNum);
      ...
      return withOperandSupplier(
              b0 ->
                  b0.operand(filterClass)
                      .predicate(MinRowNumberFilterRule::equalsOneFilter)
