...
class IcebergTableScan constructor(
...
) : TableScan(cluster, traitSet.replace(IcebergRel.CONVENTION), ImmutableList.of(), table),
    IcebergRel {
    /**
    ...
    override fun copy(
        traitSet: RelTraitSet,
        inputs: List<RelNode>,
    ): IcebergTableScan = copy(traitSet, keptColumns)

    private fun copy(
        traitSet: RelTraitSet,
        keptColumns: ImmutableBitSet,
    ): IcebergTableScan = IcebergTableScan(cluster, traitSet, table, keptColumns, catalogTable)
