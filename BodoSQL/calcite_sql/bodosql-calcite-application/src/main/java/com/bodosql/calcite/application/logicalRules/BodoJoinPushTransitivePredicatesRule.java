...
/**
 * Bodo copy of JOIN_PUSH_TRANSITIVE_PREDICATES, fixed to avoid
 * https://bodo.atlassian.net/browse/BSE-1402.
 */
@BodoSQLStyleImmutable
@Value.Enclosing
public class BodoJoinPushTransitivePredicatesRule
    extends RelRule<BodoJoinPushTransitivePredicatesRule.Config> implements TransformationRule {
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    // BODO CHANGE: added this try/catch block
    RelNode newRel;
    try {
      ...
      // if we encounter an error, do nothing, just return
      return;
    }
  ...
  public interface Config extends RelRule.Config {
    Config DEFAULT =
        ImmutableBodoJoinPushTransitivePredicatesRule.Config.of().withOperandFor(Join.class);

    @Override
    default BodoJoinPushTransitivePredicatesRule toRule() {
      return new BodoJoinPushTransitivePredicatesRule(this);
    }

    /** Defines an operand tree for the given classes. */
    default Config withOperandFor(Class<? extends Join> joinClass) {
      return withOperandSupplier(b -> b.operand(joinClass).anyInputs()).as(Config.class);
    }
  }
}
