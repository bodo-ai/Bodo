...
public class LogicVisitor extends RexUnaryBiVisitor<@Nullable Logic> {
    ...
    @Override public @Nullable Logic visitOver(RexOver over, @Nullable Logic arg) {
        // Bodo Change: Also recurse over the window sections and operands
        RexWindow window = over.getWindow();
        for (RexNode partitionKey: window.partitionKeys) {
            // Set logic to Logic.TRUE_FALSE_UNKNOWN because the partition by
            // needs all 3 values for partitioning.
            partitionKey.accept(this, Logic.TRUE_FALSE_UNKNOWN);
        }
        for (RexFieldCollation orderKey: window.orderKeys) {
            // Set logic to Logic.TRUE_FALSE_UNKNOWN because the order by
            // needs all 3 values for ordering.
            orderKey.getKey().accept(this, Logic.TRUE_FALSE_UNKNOWN);
        }
        for (RexNode node: over.getOperands()) {
            // Set logic to Logic.TRUE_FALSE_UNKNOWN because the operands
            // may need all 3 values for evaluation, regardless of if this
            // is in another node.
            node.accept(this, Logic.TRUE_FALSE_UNKNOWN);
        }
        return end(over, arg);
    }
