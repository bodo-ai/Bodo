...
public class SnowflakeUserDefinedTableFunction extends SnowflakeUserDefinedBaseFunction implements TableFunction {
     ...
     */
    protected SnowflakeUserDefinedTableFunction(ImmutableList<String> functionPath, String args, int numOptional, String returns, @Nullable String body, boolean isSecure, boolean isExternal, String language, boolean isMemoizable, java.sql.Timestamp createdOn) {
        super("UDTF", functionPath, args, numOptional, body, isSecure, isExternal, language, isMemoizable, createdOn);
        this.returnTypeInfo = parseReturnString(returns);
    }
     ...
     */
    private List<Pair<String, ColumnDataTypeInfo>> parseReturnString(String returns) {
    ...
    "Error encountered while resolving %s.%s.%s. Describe Function's \"returns\" metadata is improperly formatted or contains a type " +
        ...
        for (String column: columnParts) {
            ...
            if (parts.length != 2) {
                ...
                String name = parts[0];
                String type = parts[1];
                ColumnDataTypeInfo dataTypeInfo = snowflakeTypeNameToTypeInfo(type, true);
    ...
    // -- static creators
    public static SnowflakeUserDefinedTableFunction create(ImmutableList<String> functionPath, String args, int numOptional, String returns, @Nullable String body, boolean isSecure, boolean isExternal, String language, boolean isMemoizable, java.sql.Timestamp createdOn) {
        return new SnowflakeUserDefinedTableFunction(functionPath, args, numOptional, returns, body, isSecure, isExternal, language, isMemoizable, createdOn);
    }
