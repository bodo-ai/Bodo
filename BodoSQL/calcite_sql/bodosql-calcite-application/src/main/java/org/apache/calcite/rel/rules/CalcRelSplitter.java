...
public abstract class CalcRelSplitter {
    ...
    private int chooseLevels(
        ...
        if (levelCount > 0) {
            ...
            // BODO CHANGE: remove unnecessary level count increment since it
            // causes bugs with queries like `ROW_COUNT() OVER (ORDER BY NULL)`
             if (levelTypeOrdinals[levelCount - 1] != 0 && levelCount < exprs.length - 1) {
                 levelCount++;
             }
        }
        return levelCount;
