...
public class RexToLixTranslator implements RexVisitor<RexToLixTranslator.Result> {
  ...
  private Expression getConvertExpression(
    ...
    // Bodo Change: Add string to binary casting. By default, this will do an illicit cast
    // and crash.
    case BINARY:
    case VARBINARY:
      switch (sourceType.getSqlTypeName()) {
        case CHAR:
        case VARCHAR:
          // By default, TO_BINARY converts to hex
          Method converter = Types.lookupMethod(ByteString.class, "of", String.class, int.class);
          return Expressions.call(converter, operand, Expressions.constant(16, int.class));
        default:
          return defaultExpression.get();
      }
