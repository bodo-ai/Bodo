...
import com.bodosql.calcite.sql.type.DelegatingCustomConsistencySqlOperandTypeChecker;
...
public class SqlStdOperatorTable extends ReflectiveSqlOperatorTable {
     ...
     * of float and decimal. Note: This doesn't cast any integer inputs to float, as that would be unnecessary
     * overhead.
     * TODO: Should this be extended to other numeric operators?
     * @param inputTypes
     * @param typeFactory
     * @return
     */
    private static List<? extends RelDataType> multiplyDecimalCasting(List<? extends RelDataType> inputTypes, RelDataTypeFactory typeFactory) {
        boolean seenDecimal = false;
        boolean seenFloat = false;
        SqlTypeName floatingType = SqlTypeName.FLOAT;
        for (RelDataType type : inputTypes) {
            if (SqlTypeUtil.isDecimal(type)) {
                seenDecimal = true;
            } else if (SqlTypeUtil.isApproximateNumeric(type)) {
                seenFloat = true;
                if (type.getSqlTypeName() == SqlTypeName.DOUBLE) {
                    floatingType = SqlTypeName.DOUBLE;
                }
            }
        }
        if (seenDecimal && seenFloat) {
            List<RelDataType> outputTypes = new ArrayList<>();
            for (RelDataType type : inputTypes) {
                if (SqlTypeUtil.isDecimal(type)) {
                    outputTypes.add(typeFactory.createTypeWithNullability(typeFactory.createSqlType(floatingType), type.isNullable()));
                } else {
    ...
    public static final SqlBinaryOperator MULTIPLY =
            new SqlMonotonicBinaryOperator(
                    ...
                    InferTypes.FIRST_KNOWN,
                    new DelegatingCustomConsistencySqlOperandTypeChecker(
                            OperandTypes.MULTIPLY_OPERATOR,
                            (List<? extends RelDataType> inputTypes, RelDataTypeFactory typeFactory) ->
                                    multiplyDecimalCasting(inputTypes, typeFactory)
                    )
            );
