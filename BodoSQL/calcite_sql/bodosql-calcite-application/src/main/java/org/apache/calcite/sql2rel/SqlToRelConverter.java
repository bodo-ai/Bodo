...
public class SqlToRelConverter {
  ...
  protected void convertSelectImpl(
     ...
     * then generating a wrapping filter immediately after in convertQualifyBodo.
     */
  ...
  /**
   * The translation of 'SELECT ... FROM ... [GROUP BY ...] [HAVING ...] [QUALIFY ...]' uses
  ...
  private @Nullable RexNode convertInToOr(
    ...
    if (op instanceof SqlQuantifyOperator) {
      // Bodo Change: Use BodoRelOptUtil.op
      comparisonOp =
            BodoRelOptUtil.op(((SqlQuantifyOperator) op).comparisonKind,
  ...
  protected void convertFrom(
    ...
    case LATERAL:
      ...
      convertFrom(bb, callLateral, fieldNames, propagatingHints);
      return;
   ...
   */
  // Bodo Change: Take a SqlJoin instead of condition
  // and get the condition from the join
  private Pair<RexNode, RelNode> convertOnCondition(
      Blackboard bb,
      SqlJoin join,
      RelNode leftRel,
      RelNode rightRel) {
    SqlNode condition = join.getJoinType() != JoinType.ASOF && join.getJoinType() != JoinType.LEFT_ASOF ?
    ...
    // Bodo Change: We have modified this code to better account for subqueries. In particular,
    // our changes fix a bug related to using IN within an ON condition.
    findSubQueries(bb, condition, RelOptUtil.Logic.UNKNOWN_AS_FALSE, false, null);
    for (SubQuery node : bb.subQueryList) {
    ...
    if (conditionExp instanceof RexInputRef && curRightRel != rightRel) {
      int leftFieldCount = leftRel.getRowType().getFieldCount();
      List<RelDataTypeField> rightFieldList = curRightRel.getRowType().getFieldList();
      int rightFieldCount = curRightRel.getRowType().getFieldCount();
  ...
  protected void convertAgg(Blackboard bb, SqlSelect select,
    ...
    createAggImpl(bb, aggConverter, selectList, groupList, having,
        orderExprList, select.getQualify() != null);
  }
  ...
  private void convertQualifyBodo(Blackboard bb, int qualifyInsertionIdx) {
    ...
    final RexNode filterValWithoutNullability =
        RexUtil.removeNullabilityCast(typeFactory, filterVal);
  ...
  // Bodo Change: convertQualify is unused by Bodo because there are gaps with
  // alias expanding for group by.
  private void convertQualify(Blackboard bb, @Nullable SqlNode qualify) {
    ...
    if (qualifyWithReferencesRexNode.equals(qualifyRexNode)) {
      ...
      relBuilder.push(projectionFromSelect.getInput())
          .project(
              // Bodo Change: Remove append because mixing all RexInputRefs
              // with a call leads to an exception.
              Stream.concat(projectionFromSelect.getProjects().stream(), Stream.of(qualifyRexNode))
                          .collect(Collectors.toList()),
      ...
      relBuilder.push(requireNonNull(bb.root, "root"))
          .project(
              // Bodo Change: Remove append because mixing all RexInputRefs
              // with a call leads to an exception.
              Stream.concat(relBuilder.fields().stream(), Stream.of(qualifyWithReferencesRexNode))
                          .collect(Collectors.toList()),
  ...
  private void convertValuesImpl(
    ...
    for (SqlNode rowConstructor : values.getOperandList()) {
      ...
      Ord.forEach(newRowConst.getOperandList(), (operand, i) -> {
        ...
        if (!(def instanceof RexDynamicParam) && !def.getType().equals(fields.get(i).getType())) {
          def = rexBuilder.makeCast(operand.getParserPosition(), fields.get(i).getType(), def);
        }
    ...
    private boolean isConvertedSubq(RexNode rex) {
      ...
      if (rex instanceof RexCall) {
        ...
        if (call.getOperator() == SqlStdOperatorTable.CAST
            || call.getOperator() == SqlLibraryOperators.TRY_CAST) {
