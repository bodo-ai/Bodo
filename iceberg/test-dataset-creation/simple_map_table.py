...
df = pd.DataFrame(
    {
        ...
        "B": pd.Series([{10: Decimal(5.6)}, {65: Decimal(34.6)}] * 25, dtype=object),
        "C": pd.Series([{"ERT": 10.0}, {"ASD": 23.87}] * 25, dtype=object),
        "D": pd.Series([{Decimal(54.67): 54}, {Decimal(32.90): 32}] * 25, dtype=object),
    }
)
schema = spark_types.StructType(
    [
        spark_types.StructField(
            "A",
            spark_types.MapType(spark_types.StringType(), spark_types.LongType(), True),
        ),
        spark_types.StructField(
            "B",
            spark_types.MapType(
...
create_iceberg_table(df, schema, "simple_map_table")
