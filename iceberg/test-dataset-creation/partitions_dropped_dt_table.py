...
"""
Similar to partitions_dt_table except intermediate partitions are
dropped rather than just updated, changing what can be determined
...
spark.sql(
    f"""
    ALTER TABLE hadoop_prod.iceberg_db.{table_name}
...
print("Modifying partition field (year --> month)..., deleting year")
spark.sql(
    f"""
    ALTER TABLE hadoop_prod.iceberg_db.{table_name}
