...
/**
 * @brief Returns whether it is valid to call
 * compute_window_functions_via_calculators on a set of window functions with a
 * specific number of partition/order indices.
 */
inline bool supports_calculator_computation(
    const std::vector<int32_t> &partition_col_indices,
    const std::vector<int32_t> &order_col_indices,
    const std::vector<int32_t> &window_funcs) {
    // Ensure we are not using the calculator logic when we should be going to
    // the OVER () infrastructure.
    if ((partition_col_indices.size()) == 0 &&
        (order_col_indices.size() == 0)) {
        return false;
    }
