...
std::shared_ptr<v8::Isolate> create_new_isolate() {
    ...
    return std::shared_ptr<v8::Isolate>(isolate, [](v8::Isolate *isolate) {isolate->Dispose();});
}
...
std::unique_ptr<JavaScriptFunction> JavaScriptFunction::create(
    ...
    std::unique_ptr<JavaScriptFunction> f =
        std::unique_ptr<JavaScriptFunction>(new JavaScriptFunction(
            _body, _arg_names, std::move(_return_type)));
    return f;
}
...
JavaScriptFunction::JavaScriptFunction(
    ...
    std::string script_str = fmt::format(
        "function {}({}) {{ {} }}", this->random_function_name,
        fmt::join(this->arg_names, ", "), _body);
    v8::Local<v8::String> source =
        v8::String::NewFromUtf8(isolate.get(), script_str.c_str())
            .ToLocalChecked();
    v8::Local<v8::Script> script = v8::Script::Compile(this->context.Get(isolate.get()), source).ToLocalChecked();
    script->Run(this->context.Get(isolate.get())).ToLocalChecked();


     v8::Local<v8::String> v8_func_name =
        v8::String::NewFromUtf8(isolate.get(), this->random_function_name.c_str())
            .ToLocalChecked();
    v8::Local<v8::Context> local_context = this->context.Get(isolate.get());
    v8::Local<v8::Value> v8_func_value = local_context->Global()->Get(local_context, v8_func_name).ToLocalChecked();
    assert(v8_func_value->IsFunction());
    this->v8_func.Reset(isolate.get(), v8::Local<v8::Function>::Cast(v8_func_value));
...
JavaScriptFunction *create_javascript_udf_py_entry(
    ...
    try {
        ...
        arg_names_vec.reserve(nargs);
        for (size_t i = 0; i < nargs; ++i) {
            arg_names_vec.push_back(
                std::string(arg_names[arg_names_offsets[i]],
                            arg_names_offsets[i + 1] - arg_names_offsets[i]));
        }
        JavaScriptFunction *f = new JavaScriptFunction(
            std::string(body, len_body), arg_names_vec,
...
std::shared_ptr<array_info> execute_javascript_udf(
    ...
    std::shared_ptr<array_info> ret = alloc_array_top_level(out_count, 0, 0, func->return_type->array_type, func->return_type->c_type);
    for (size_t i = 0;i < out_count; ++i) {
    auto result = local_v8_func->Call(local_context, local_context->Global(), 0, nullptr)
                      .ToLocalChecked();
        int64_t retval = result->IntegerValue(local_context).ToChecked();
        // this needs to become something like cast_v8_value_to_bodo_array<array_type, ctype>(result);
        ret->data1()[i] = retval;
    }
