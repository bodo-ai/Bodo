...
void nth_sequential(void *res, void *data, int64_t local_size, int64_t k,
                    int type_enum);
void nth_parallel(void *res, void *data, int64_t local_size, int64_t k,
                  int type_enum);
...
// MPI_Allreduce(&my_sample_size, &total_sample_size, 1, MPI_LONG_LONG_INT,
MPI_SUM, MPI_COMM_WORLD);
*/

template <class T>
void get_nth(T *res, T *data, int64_t local_size, int64_t k, int type_enum,
             int myrank, int n_pes, bool parallel) {
    // get nth element and store in res pointer
    // assuming NA values of floats are already removed
    std::vector<T> my_array(data, data + local_size);
    T val;
...
void nth_dispatch(void *res, void *data, int64_t local_size, int64_t k,
                  int type_enum, bool parallel) {
    int myrank, n_pes;
    MPI_Comm_size(MPI_COMM_WORLD, &n_pes);
    MPI_Comm_rank(MPI_COMM_WORLD, &myrank);
    ...
    switch (type_enum) {
        ...
        case Bodo_CTypes::INT32:
            return get_nth((int *)res, (int *)data, local_size, k, type_enum,
                           myrank, n_pes, parallel);
        case Bodo_CTypes::UINT32:
            return get_nth((uint32_t *)res, (uint32_t *)data, local_size, k,
                           type_enum, myrank, n_pes, parallel);
        case Bodo_CTypes::INT64:
            return get_nth((int64_t *)res, (int64_t *)data, local_size, k,
                           type_enum, myrank, n_pes, parallel);
        case Bodo_CTypes::UINT64:
            return get_nth((uint64_t *)res, (uint64_t *)data, local_size, k,
                           type_enum, myrank, n_pes, parallel);
        case Bodo_CTypes::FLOAT32:
            return get_nth((float *)res, (float *)data, local_size, k,
                           type_enum, myrank, n_pes, parallel);
        case Bodo_CTypes::FLOAT64:
            return get_nth((double *)res, (double *)data, local_size, k,
                           type_enum, myrank, n_pes, parallel);
        default:
