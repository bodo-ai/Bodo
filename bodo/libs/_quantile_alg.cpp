 ...
 */
void autocorr_series_computation(double* res, array_info* arr, int64_t lag, bool is_parallel);


/* Compute whether the array is monotonic or not
   @output res : the return value 1 for monotonic and 0 otherwise
   @param arr : the input array
   @param inc_dec : 1 for testing is_monotonic_increasing / 2 for is_monotonic_decreasing
   @param parallel: whether it is parallel or not
 */
void compute_series_monotonicity(double* res, array_info* arr, int64_t inc_dec, bool is_parallel);
...
/* The computation of several operations requires one to know the rows
   ...
   the point. The algorithm is waterproof in the sense that any depth is possible.
   We may need to use one level of next entries or more but the algorithm will
   handle that.
   ----
   Limitations:
...
array_info* compute_ghost_rows(array_info* arr, uint64_t const& level_next)
{
    ...
    // We do not share all the number of rows. Only the ones that are just previous
    // and next. We stop when ListPrevSizes[r-1] + ... + ListPrevSizes[r-k] <= lag
    // on all nodes. For the nodes of rank 0 the vectors are smaller of course.
    ...
    while(true) {
        k++;
#ifdef DEBUG_GHOST_CODE
        std::cout << "Beginning of while loop k=" << k << " loc_nrows=" << loc_nrows << "\n";
#endif
        ...
        if (myrank >= k) {
            ...
            int tag = 10000 + pe;
            MPI_Isend((void*)ptr_T, 1, mpi_row_typ, pe, tag, MPI_COMM_WORLD, &mpi_send_next);
        ...
        if (myrank < n_pes - k) {
            ...
            V[idx] = loc_nrows;
            T_row_typ* ptr_T = V.data() + idx;
            MPI_Isend((void*)ptr_T, 1, mpi_row_typ, pe, tag, MPI_COMM_WORLD, &mpi_send_prev);
        ...
        if (myrank < n_pes - k) {
            ...
            T_row_typ* ptr_T = V.data() + idx;
            MPI_Irecv((void*)ptr_T, 1, mpi_row_typ, pe, tag, MPI_COMM_WORLD, &mpi_recv_next);
            idx_recv_next = idx;
        ...
        if (myrank >= k) {
            ...
            T_row_typ* ptr_T = V.data() + idx;
            MPI_Irecv((void*)ptr_T, 1, mpi_row_typ, pe, tag, MPI_COMM_WORLD, &mpi_recv_prev);
            idx_recv_prev = idx;
        ...
        size_t sumprev = std::accumulate(ListPrevSizes.begin(), ListPrevSizes.end(), 0);
        size_t sumnext = std::accumulate(ListNextSizes.begin(), ListNextSizes.end(), 0);
#ifdef DEBUG_GHOST_CODE
        ...
        // value=0 means all is ok. All nodes should be ok for the loop to terminate
#ifdef DEBUG_GHOST_CODE
    ...
    for (int i_next=0; i_next<n_next; i_next++)
      std::cout << "i_next=" << i_next << " / " << n_next << " NextSize=" << ListNextSizes[i_next] << "\n";
    for (int i_prev=0; i_prev<n_prev; i_prev++)
      std::cout << "i_prev=" << i_prev << " / " << n_prev << " PrevSize=" << ListPrevSizes[i_prev] << "\n";
    std::cout << "--------------------------------------------------\n";
#endif
...
#ifdef DEBUG_GHOST_CODE
        std::cout << "i_next=" << i_next << " siz=" << siz << " siz_recv=" << siz_recv << "\n";
#endif
        if (siz_recv > 0) {
...
#endif
            MPI_Irecv((void*)ptr_recv, siz_recv, mpi_typ, pe, tag, MPI_COMM_WORLD, &mpi_recv);
...
#ifdef DEBUG_GHOST_CODE
        std::cout << "i_prev=" << i_prev << " loc_nrows=" << loc_nrows << " siz_send=" << siz_send << "\n";
#endif
        if (siz_send > 0) {
...
#endif
            MPI_Isend((void*)ptr_send, siz_send, mpi_typ, pe, tag, MPI_COMM_WORLD, &mpi_send);
...
#ifdef DEBUG_GHOST_CODE
    std::vector<std::string> ListStrRow = DEBUG_PrintColumn(ghost_arr);
    std::cout << "ghost_length=" << ghost_length << "\n";
    for (uint64_t i=0; i<ghost_length; i++)
        std::cout << "i=" << i << " row=" << ListStrRow[i] << "\n";
...
{
#ifdef DEBUG_GHOST_CODE
    std::cout << "compute_monotonicity : inc_dec=" << inc_dec << " is_parallel=" << is_parallel << "\n";
#endif
...
#endif
    if (value_tot > 0) { // At least one node find a contradiction locally. Enough to conclude
        *res = 0.0;
        return;
    }
    ...
    if (ghost_arr->length > 0) { // It will be empty on the last node and maybe others.
    ...
    MPI_Allreduce(&value_glob, &value_glob_tot, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    if (value_glob_tot > 0) {
        *res = 0.0;
    } else {
        *res = 1.0;
...
{
#ifdef DEBUG_GHOST_CODE
    std::cout << "autocorr_series_computation : lag=" << lag << " is_parallel=" << is_parallel << "\n";
#endif
    uint64_t n_rows = arr->length;
    uint64_t siztype = numpy_item_size[arr->dtype];
#ifdef DEBUG_GHOST_CODE
    std::cout << "n_rows=" << n_rows << " dtype=" << arr->dtype << " siztype=" << siztype << "\n";
#endif
    if (!is_parallel) {
...
#ifdef DEBUG_GHOST_CODE
            std::cout << "i_row=" << i_row << " val1=" << val1 << " val2=" << val2 << "\n";
#endif
            sum1 += val1;
            sum2 += val2;
            sum12 += val1 * val2;
    ...
    uint64_t n_rows_tot;
    MPI_Allreduce(&n_rows, &n_rows_tot, 1, MPI_UNSIGNED_LONG_LONG, MPI_SUM, MPI_COMM_WORLD);
#ifdef DEBUG_GHOST_CODE
    ...
    if (n_rows + ghost_siz >= uint64_t(lag)) {
        uint64_t n_rows_cons = n_rows + ghost_siz - lag; // the ghost may provide the additional rows or may not
#ifdef DEBUG_GHOST_CODE
        std::cout << "n_rows_cons=" << n_rows_cons << " n_rows=" << n_rows << " lag=" << lag << " ghost_siz=" << ghost_siz << "\n";
#endif
...
#ifdef DEBUG_GHOST_CODE
    std::cout << "LOC: sum1=" << sum1 << " sum2=" << sum2 << " sum11=" << sum11 << " sum12=" << sum12 << " sum22=" << sum22 << "\n";
#endif
    std::vector<double> Vtot(5);
    MPI_Allreduce(V.data(), Vtot.data(), 5, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
#ifdef DEBUG_GHOST_CODE
    std::cout << "GLB: sum1=" << Vtot[0] << " sum2=" << Vtot[1] << " sum11=" << Vtot[2] << " sum12=" << Vtot[3] << " sum22=" << Vtot[4] << "\n";
#endif
