...
// ---We can use shuffle-join or broadcast-join. We have one constant for the maximal
//    size of the broadcasted table. It is set to 1024*1024, while in spark it is by
//    default 10 * 1024 * 1024. Variable name "CritMemorySize".
// ---For the join, we need to construct one hash map for the keys. If we take the left
//    table and is_left=T then we need to build a more complicated hash-map. Thus
//    the size of the table is just one parameter in the choice.
...
# endif
    ...
    // ---Shuffle join requires shuffling which implies the use of MPI_alltoallv
    //    and uses a lot of memory.
    // ---Broadcast join requires one table to be gathered which may be advantageous
    //    if one table is much smaller. Still making a table replicated should give
    //    pause to users.
    //
    ...
    if (left_parallel && right_parallel) {
        ...
        int CritMemorySize = 1024 * 1024;  // 1M in total is the critical size
...
#ifdef DEBUG_JOIN_SYMBOL
    std::cout << "left_replicated=" << left_replicated << " right_replicated=" << right_replicated << "\n";
#endif
...
# endif
...
#endif
    // In the computation of the join we are building some hash map on one of
    // the tables. Which one we choose influence the running time of course.
    // --- Selecting the smaller table is a good heuristic for the construction,
    // the smaller the table, the smaller the hash map.
    // --- Selecting one table for going into the hash-map while its values show
    // up in the output for example via an outer merge means that we need to
    // keep track of which keys have been used or not. This complexifies the
    // data structure.
    //
    // Thus we use what we have to make the best choice about the computational
    // technique.
    ...
    if (is_left == is_right) {
        ...
        // In the case of is_left <> is_right we have a conflict regarding how
        // to choose We decide to set up an heuristic about this for making the
        // decision
        double CritQuotientNrows = 6.0;
        if (quot2 < CritQuotientNrows && quot1 < CritQuotientNrows)
            // In that case the large table is not so large comparable to the
            // short one This means that we can use the is_left / is_right for
            // making the choice
            MethodChoice = 2;
        else
            // In that case one table is much larger than the other, therefore
            // the choice by the number of rows is the best here.
            MethodChoice = 1;
    }
    ...
    std::cout << "MethodChoice=" << MethodChoice << " ChoiceOpt=" << ChoiceOpt
              << "\n";
#endif
    // Now that we have made a choice opt, we set what is the "short" table and the "long" one.
    // For the short table we construct a hash map, for the long table, we simply iterate over
    // the rows and see if the keys are in the hash map.
    ...
    // If one table (say left one) is replicated and the other is distributed and is_left=T
    // then we need to keep track of how the rows of the left table are matched.
    bool parallel_track_miss_short = short_table_work && short_replicated && !long_replicated;
    bool parallel_track_miss_long = long_table_work && long_replicated && !short_replicated;
#ifdef DEBUG_JOIN_SYMBOL
    ...
    // Now running according to the short_table_work. The choice depends on the
    // initial choices that have been made.
    size_t n_bytes_long;
        ...
        for (auto& group : entList) {
            ...
            if (!bit) {
                for (size_t idx = 1; idx < group.second.size(); idx++) {
                    ...
                    // For parallel_track_miss_short=T the output table is distributed.
                    // Since the table in input is replicated, we dispatch it by rank.
                    if (parallel_track_miss_short) {
    ...
    if (long_table_work && parallel_track_miss_long) {
        ...
        for (size_t i_long = 0; i_long < long_table_rows; i_long++) {
            ...
            // The replicated input table is dispatched over the rows in the distributed output.
            if (!bit) {
                int node = pos % n_pes;
                if (node == myrank)
                    ListPairWrite.push_back({-1, i_long});
    ...
    for (size_t i = 0; i < n_tot_left; i++) {
        if (i < n_key && vect_same_key[i < n_key ? i : 0] == 1) {
            ...
            if (ChoiceOpt == 0) {
                out_arrs.emplace_back(RetrieveArray_TwoColumns(
                    left_arr, right_arr, ListPairWrite, 0, map_integer_type));
            } else {
                out_arrs.emplace_back(RetrieveArray_TwoColumns(
                    right_arr, left_arr, ListPairWrite, 1, map_integer_type));
            }
    ...
    for (size_t i = 0; i < n_tot_right; i++) {
        ...
        if (i >= n_key ||
            ...
            if (ChoiceOpt == 0) {
                out_arrs.emplace_back(RetrieveArray_TwoColumns(
                    left_arr, right_arr, ListPairWrite, 1, map_integer_type));
            } else {
                out_arrs.emplace_back(RetrieveArray_TwoColumns(
                    right_arr, left_arr, ListPairWrite, 0, map_integer_type));
            }
            idx++;
