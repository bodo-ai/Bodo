...
BasicColSet::BasicColSet(array_info* in_col, int ftype, bool combine_step,
                         bool use_sql_rules)
    : in_col(in_col),
      ftype(ftype),
...
void BasicColSet::alloc_update_columns(size_t num_groups,
...
typename std::vector<array_info*>::iterator BasicColSet::update_after_shuffle(
...
void BasicColSet::alloc_combine_columns(size_t num_groups,
                                        std::vector<array_info*>& out_cols) {
...
array_info* BasicColSet::getOutputColumn() {
...
MeanColSet::MeanColSet(array_info* in_col, bool combine_step,
                       bool use_sql_rules)
    : BasicColSet(in_col, Bodo_FTypes::mean, combine_step, use_sql_rules) {}

MeanColSet::~MeanColSet() {}

void MeanColSet::alloc_update_columns(size_t num_groups,
                                      std::vector<array_info*>& out_cols) {
    array_info* c1 =
...
IdxMinMaxColSet::IdxMinMaxColSet(array_info* in_col, array_info* _index_col,
...
void IdxMinMaxColSet::alloc_update_columns(size_t num_groups,
                                           std::vector<array_info*>& out_cols) {
    ...
    array_info* max_col = alloc_array(num_groups, 1, 1, this->in_col->arr_type,
                                      this->in_col->dtype, 0,
                                      0);  // for min/max
    // create array to store index position of min/max value
    array_info* index_pos_col = alloc_array(
...
void IdxMinMaxColSet::alloc_combine_columns(
    size_t num_groups, std::vector<array_info*>& out_cols) {
    // output column containing index values. dummy for now. will be
...
VarStdColSet::VarStdColSet(array_info* in_col, int ftype, bool combine_step,
                           bool use_sql_rules)
    : BasicColSet(in_col, ftype, combine_step, use_sql_rules) {}

VarStdColSet::~VarStdColSet() {}

void VarStdColSet::alloc_update_columns(size_t num_groups,
                                        std::vector<array_info*>& out_cols) {
    if (!this->combine_step) {
...
void VarStdColSet::alloc_combine_columns(size_t num_groups,
                                         std::vector<array_info*>& out_cols) {
    array_info* col =
        alloc_array(num_groups, 1, 1, bodo_array_type::NULLABLE_INT_BOOL,
                    Bodo_CTypes::FLOAT64, 0,
...
UdfColSet::UdfColSet(array_info* in_col, bool combine_step,
                     table_info* udf_table, int udf_table_idx, int n_redvars,
                     bool use_sql_rules)
...
void UdfColSet::alloc_update_columns(size_t num_groups,
                                     std::vector<array_info*>& out_cols) {
    int offset = 0;
...
typename std::vector<array_info*>::iterator UdfColSet::update_after_shuffle(
    typename std::vector<array_info*>::iterator& it) {
    // UdfColSet doesn't keep the update cols, return the updated iterator
    return it + n_redvars;
}

void UdfColSet::alloc_combine_columns(size_t num_groups,
                                      std::vector<array_info*>& out_cols) {
...
GeneralUdfColSet::GeneralUdfColSet(array_info* in_col, table_info* udf_table,
...
void GeneralUdfColSet::fill_in_columns(table_info* general_in_table,
...
MedianColSet::MedianColSet(array_info* in_col, bool _skipna, bool use_sql_rules)
...
NUniqueColSet::NUniqueColSet(array_info* in_col, bool _dropna,
                             table_info* nunique_table, bool do_combine,
...
CumOpColSet::CumOpColSet(array_info* in_col, int ftype, bool _skipna,
                         bool use_sql_rules)
    : BasicColSet(in_col, ftype, false, use_sql_rules), skipna(_skipna) {}

CumOpColSet::~CumOpColSet() {}

void CumOpColSet::alloc_update_columns(size_t num_groups,
                                       std::vector<array_info*>& out_cols) {
...
ShiftColSet::ShiftColSet(array_info* in_col, int ftype, int64_t _periods,
                         bool use_sql_rules)
    : BasicColSet(in_col, ftype, false, use_sql_rules), periods(_periods) {}
ShiftColSet::~ShiftColSet() {}

void ShiftColSet::alloc_update_columns(size_t num_groups,
                                       std::vector<array_info*>& out_cols) {
...
TransformColSet::TransformColSet(array_info* in_col, int ftype, int _func_num,
...
void TransformColSet::alloc_update_columns(size_t num_groups,
                                           std::vector<array_info*>& out_cols) {
...
void HeadColSet::alloc_update_columns(size_t update_col_len,
                                      std::vector<array_info*>& out_cols) {
...
NgroupColSet::NgroupColSet(array_info* in_col, bool _is_parallel,
                           bool use_sql_rules)
...
void NgroupColSet::alloc_update_columns(size_t num_groups,
                                        std::vector<array_info*>& out_cols) {
    bodo_array_type::arr_type_enum arr_type = this->in_col->arr_type;
    Bodo_CTypes::CTypeEnum dtype = this->in_col->dtype;
    int64_t num_categories = this->in_col->num_categories;
