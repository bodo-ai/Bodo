...
bool set_itervalid_string(std::unordered_set<std::string>::iterator* itp,
...
PyMODINIT_FUNC PyInit_hset_ext(void) {
    ...
    PyObject_SetAttrString(
        m, "num_total_chars_set_string",
        PyLong_FromVoidPtr((void*)(&num_total_chars_set_string)));
    PyObject_SetAttrString(
        m, "populate_str_arr_from_set",
...
void* set_iterator_string(std::unordered_set<std::string>* str_set) {
    std::unordered_set<std::string>::iterator* itp =
        new std::unordered_set<std::string>::iterator(str_set->begin());
    return itp;
}
