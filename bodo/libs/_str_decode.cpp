...
#define _C_UnicodeWriter_Prepare(WRITER, LENGTH, MAXCHAR)           \
    (((MAXCHAR) <= (WRITER)->maxchar &&                             \
      (LENGTH) <= (WRITER)->size - (WRITER)->pos)                   \
         ? 0                                                        \
         : (((LENGTH) == 0) ? 0 : _C_UnicodeWriter_PrepareInternal( \
...
static inline int _C_UnicodeWriter_WriteCharInline(_C_UnicodeWriter *writer,
                                                   Py_UCS4 ch);
static int _copy_characters(NRT_MemInfo *to, Py_ssize_t to_start,
                            NRT_MemInfo *from, Py_ssize_t from_start,
                            Py_ssize_t how_many, unsigned int from_kind,
                            unsigned int to_kind);

// similar to PyUnicode_New()
NRT_MemInfo *alloc_writer(_C_UnicodeWriter *writer, Py_ssize_t newlen,
                          Py_UCS4 maxchar) {
    enum PyUnicode_Kind kind;
    int is_ascii = 0;
    ...
    if (writer->buffer != NULL) {
        _copy_characters(newbuffer, 0, writer->buffer, 0, writer->pos,
                         writer->kind, kind);
...
int _C_UnicodeWriter_PrepareInternal(_C_UnicodeWriter *writer,
    ...
    if (writer->buffer == NULL) {
        ...
        if (newlen < writer->min_length) newlen = writer->min_length;

        writer->buffer = alloc_writer(writer, newlen, maxchar);
        if (writer->buffer == NULL) return -1;
...
static Py_ssize_t ascii_decode(const char *start, const char *end,
                               Py_UCS1 *dest) {
    const char *p = start;
    ...
    if (_Py_IS_ALIGNED(p, SIZEOF_LONG)) {
        ...
        while (_p < aligned_end) {
            ...
            if (value & ASCII_CHAR_MASK) break;
...
// ported from CPython PyUnicode_DecodeUTF8Stateful:
// https://github.com/python/cpython/blob/31e8d69bfe7cf5d4ffe0967cb225d2a8a229cc97/Objects/unicodeobject.c#L4813
void decode_utf8(const char *s, Py_ssize_t size, int *kind, int *is_ascii,
                 int *length, NRT_MemInfo **meminfo) {
    ...
    while (s < end) {
        ...
        if (kind == PyUnicode_1BYTE_KIND) {
            if (writer.is_ascii == 1)
                ch = asciilib_utf8_decode(&s, end, (Py_UCS1 *)writer.data,
                                          &writer.pos);
            else
                ch = ucs1lib_utf8_decode(&s, end, (Py_UCS1 *)writer.data,
                                         &writer.pos);
        } else if (kind == PyUnicode_2BYTE_KIND) {
            ch = ucs2lib_utf8_decode(&s, end, (Py_UCS2 *)writer.data,
                                     &writer.pos);
        } else {
            assert(kind == PyUnicode_4BYTE_KIND);
            ch = ucs4lib_utf8_decode(&s, end, (Py_UCS4 *)writer.data,
                                     &writer.pos);
        }
...
static int _copy_characters(NRT_MemInfo *to, Py_ssize_t to_start,
    ...
    if (from_kind == to_kind) {
        ...
        _PyUnicode_CONVERT_BYTES(
            Py_UCS1, Py_UCS2, ((Py_UCS1 *)(from_data)) + from_start,
            ((Py_UCS1 *)(from_data)) + from_start + how_many,
            ((Py_UCS2 *)(to_data)) + to_start);
    } else if (from_kind == PyUnicode_1BYTE_KIND &&
               to_kind == PyUnicode_4BYTE_KIND) {
        _PyUnicode_CONVERT_BYTES(
            Py_UCS1, Py_UCS4, ((Py_UCS1 *)(from_data)) + from_start,
            ((Py_UCS1 *)(from_data)) + from_start + how_many,
            ((Py_UCS4 *)(to_data)) + to_start);
    } else if (from_kind == PyUnicode_2BYTE_KIND &&
               to_kind == PyUnicode_4BYTE_KIND) {
        _PyUnicode_CONVERT_BYTES(
            Py_UCS2, Py_UCS4, ((Py_UCS2 *)(from_data)) + from_start,
        ...
        if (1) {
            if (from_kind == PyUnicode_2BYTE_KIND &&
                ...
                _PyUnicode_CONVERT_BYTES(
                    Py_UCS2, Py_UCS1, ((Py_UCS2 *)(from_data)) + from_start,
                    ((Py_UCS2 *)(from_data)) + from_start + how_many,
                    ((Py_UCS1 *)(to_data)) + to_start);
            } else if (from_kind == PyUnicode_4BYTE_KIND &&
                       to_kind == PyUnicode_1BYTE_KIND) {
                _PyUnicode_CONVERT_BYTES(
                    Py_UCS4, Py_UCS1, ((Py_UCS4 *)(from_data)) + from_start,
                    ((Py_UCS4 *)(from_data)) + from_start + how_many,
                    ((Py_UCS1 *)(to_data)) + to_start);
            } else if (from_kind == PyUnicode_4BYTE_KIND &&
                       to_kind == PyUnicode_2BYTE_KIND) {
                _PyUnicode_CONVERT_BYTES(
                    Py_UCS4, Py_UCS2, ((Py_UCS4 *)(from_data)) + from_start,
                    ((Py_UCS4 *)(from_data)) + from_start + how_many,
                    ((Py_UCS2 *)(to_data)) + to_start);
            } else {
                abort();
...
static inline int _C_UnicodeWriter_WriteCharInline(_C_UnicodeWriter *writer,
    ...
    if (_C_UnicodeWriter_Prepare(writer, 1, ch) < 0) return -1;
