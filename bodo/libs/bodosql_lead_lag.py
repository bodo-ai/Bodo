...
def lead_lag_seq(in_col, shift_amt, default_fill_val=None, ignore_nulls=False):
    ...
    # Handle necessary conversions for certain input types. A wrapper around value_to_ptr should perhaps exist with this functionality.
    if isinstance(default_fill_val, types.NoneType):  # pragma: no cover
        ctx["fill_val"] = "(fill_val, fill_val_len) = (0, 0)"
    elif is_valid_string_arg(default_fill_val):  # pragma: no cover
        ctx[
            "fill_val"
        ] = "(fill_val, fill_val_len) = bodo.libs.str_ext.unicode_to_utf8_and_len(default_fill_val); fill_val = value_to_ptr(fill_val)"
    elif is_valid_binary_arg(default_fill_val):
        ctx[
            "fill_val"
        ] = "(fill_val, fill_val_len) = (value_to_ptr(default_fill_val._data), len(default_fill_val))"
    elif is_valid_tz_naive_datetime_arg(
        default_fill_val
    ) or is_valid_tz_aware_datetime_arg(default_fill_val):
        ctx[
            "fill_val"
        ] = "(fill_val, fill_val_len) = (value_to_ptr(default_fill_val.value), 0)"
