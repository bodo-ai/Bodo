...
std::shared_ptr<array_info> select_subset_of_rows_numeric_operation(
    ...
    for (size_t i = 0; i < n_rows; i++) {
        ...
        if (is_null_at<ArrType, T, DType>(*arr, row_to_select)) {
            set_to_null<ArrType, T, DType>(*out_arr, i);
        } else {
            set_arr_item<ArrType, T, DType>(
                *out_arr, i,
                get_arr_item<ArrType, T, DType>(*arr, row_to_select));
        }
    }
...
#define SUBSET_OF_ROWS_NUMERIC_CASE(dtype)                              \
    ...
    switch (arr->dtype) {
        ...
        SUBSET_OF_ROWS_NUMERIC_CASE(Bodo_CTypes::UINT16)
        SUBSET_OF_ROWS_NUMERIC_CASE(Bodo_CTypes::UINT32)
        SUBSET_OF_ROWS_NUMERIC_CASE(Bodo_CTypes::UINT64)
        SUBSET_OF_ROWS_NUMERIC_CASE(Bodo_CTypes::INT8)
        SUBSET_OF_ROWS_NUMERIC_CASE(Bodo_CTypes::INT16)
        SUBSET_OF_ROWS_NUMERIC_CASE(Bodo_CTypes::INT32)
        SUBSET_OF_ROWS_NUMERIC_CASE(Bodo_CTypes::INT64)
        SUBSET_OF_ROWS_NUMERIC_CASE(Bodo_CTypes::INT128)
        SUBSET_OF_ROWS_NUMERIC_CASE(Bodo_CTypes::DECIMAL)
        SUBSET_OF_ROWS_NUMERIC_CASE(Bodo_CTypes::DATE)
        SUBSET_OF_ROWS_NUMERIC_CASE(Bodo_CTypes::TIME)
        SUBSET_OF_ROWS_NUMERIC_CASE(Bodo_CTypes::TIMEDELTA)
        SUBSET_OF_ROWS_NUMERIC_CASE(Bodo_CTypes::DATETIME)
        SUBSET_OF_ROWS_NUMERIC_CASE(Bodo_CTypes::FLOAT32)
        SUBSET_OF_ROWS_NUMERIC_CASE(Bodo_CTypes::FLOAT64)
        SUBSET_OF_ROWS_NUMERIC_CASE(Bodo_CTypes::_BOOL)
        default: {
            throw std::runtime_error(
                "select_subset_of_rows_numeric: unsupported dtype for "
                "nullable/numpy "
                "input: " +
...
std::shared_ptr<array_info> select_subset_of_rows_string(
    ...
    for (size_t i = 0; i < n_rows; i++) {
        int64_t row_to_select = rows_to_select[i];
        if (arr->get_null_bit(row_to_select)) {
            total_chars +=
                in_offsets[row_to_select + 1] - in_offsets[row_to_select];
        }
    }
    ...
    // Copy over the desired offsets and subsets of the character buffer
    offset_t curr_offset = 0;
    out_offsets[n_rows] = total_chars;
    for (size_t i = 0; i < n_rows; i++) {
        out_offsets[i] = curr_offset;
        int64_t row_to_select = rows_to_select[i];
        if (!arr->get_null_bit(row_to_select)) {
            out_arr->set_null_bit(i, false);
        } else {
            offset_t end_offset = in_offsets[row_to_select + 1];
            offset_t start_offset = in_offsets[row_to_select];
            offset_t str_len = end_offset - start_offset;
            memcpy(out_chars + curr_offset, in_chars + start_offset, str_len);
            curr_offset += str_len;
        }
    }
    return out_arr;
...
std::shared_ptr<array_info> select_subset_of_rows(
    ...
    switch (arr->arr_type) {
        ...
        default: {
            throw std::runtime_error(
                "select_subset_of_rows: unsupported array type for "
                "inner array: " +
                (GetArrType_as_string(arr->arr_type)));
        }
    }
