...
# dummy function for groupby apply() generated by dataframe pass, to be replaced by
# distributed pass
def groupby_apply(keys, in_df, map_func, out_type, *args, **kws):  # pragma: no cover
    pass


@infer_global(groupby_apply)
class GroupbyApplyTyper(AbstractTemplate):
    def generic(self, args, kws):
        kws = dict(kws)
        f_args = args[4:]
        arg_names = ", ".join(f"arg{i}" for i in range(len(f_args)))
        ...
        pysig = numba.core.utils.pysignature(apply_stub)
        new_args = args + tuple(kws.values())
        ret_type = args[3].instance_type
        return signature(ret_type, *new_args).replace(pysig=pysig)
