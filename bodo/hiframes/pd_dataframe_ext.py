...
def tuple_typer(self, args, kws):
    ...
    # Bodo change: implement getitem for row namedtuple in df.apply()
    elif (
        isinstance(idx, str)
        and isinstance(tup, types.BaseNamedTuple)
        and tup.instance_class.__module__.startswith("bodo")
    ):
        val_ind = tup.instance_class._fields.index(idx)
        ret = tup.types[val_ind]
...
numba.core.typing.builtins.StaticGetItemTuple.generic = tuple_typer


@lower_builtin("static_getitem", types.BaseTuple, types.StringLiteral)
def static_getitem_tuple_str(context, builder, sig, args):
    tupty, _ = sig.args
    tup, _ = args
    val_ind = tupty.instance_class._fields.index(sig.args[1].literal_value)
    res = builder.extract_value(tup, val_ind)
