    ...
    with temp_env_override(
        ...
        def impl(df, op_pool_size_bytes):
            ...
            while not is_last1:
                ...
                is_last1, _ = bodo.libs.stream_window.window_build_consume_batch(
                    window_state, T3, is_last1
                )
                ### Uncomment for debugging purposes ###
                # bytes_pinned = bodo.libs.stream_groupby.get_op_pool_bytes_pinned(window_state)
                # bytes_allocated = bodo.libs.stream_groupby.get_op_pool_bytes_allocated(window_state)
                # bodo.parallel_print(
            ...
            final_bytes_pinned = bodo.libs.stream_groupby.get_op_pool_bytes_pinned(
                window_state
            )
            final_bytes_allocated = (
                bodo.libs.stream_groupby.get_op_pool_bytes_allocated(window_state)
            )
            bodo.libs.stream_window.delete_window_state(window_state)
