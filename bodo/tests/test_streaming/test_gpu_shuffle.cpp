...
static bodo::tests::suite tests([] {
    ...
    bodo::tests::test(
        ...
        [] {
            ...
            auto result = hash_partition_table(
                std::shared_ptr<cudf::table>(std::move(table)), hash_cols,
                num_partitions);
    ...
    bodo::tests::test(
        "test_shuffle_integers_end_to_end",
        [] {
            int rank, n_ranks;
            ...
            CHECK_MPI(
                MPI_Allreduce(&local_received_rows, &global_received_rows, 1,
                              MPI_INT, MPI_SUM, MPI_COMM_WORLD),
                "test_shuffle_integers_end_to_end: MPI_Allreduce failed:");

            // 3. Check conservation of data: Total rows in system must equal
            // total rows out Input was rows_per_rank * number of devices
