...
def count_array_REPs():
    from bodo.transforms.distributed import Distribution
    vals = bodo.transforms.distributed.dist_analysis.array_dists.values()
    return sum([v==Distribution.REP for v in vals])

def count_parfor_REPs():
    from bodo.transforms.distributed import Distribution
    vals = bodo.transforms.distributed.dist_analysis.parfor_dists.values()
    return sum([v==Distribution.REP for v in vals])
...
def dist_IR_contains(*args):
    return sum([(s in bodo.transforms.distributed.fir_text) for s in args])
