...
def test_interval_add_time_interval_to_date(
    ...
    if isinstance(date_input, datetime.date):
        impl = lambda arr0, arr1: bodo.libs.bodosql_array_kernels.add_interval(
            arr0, arr1
        )
...
def test_interval_add_date_interval_to_date(
    ...
    def impl(arr0, arr1):
        return pd.Series(
            bodo.libs.bodosql_array_kernels.add_date_interval_to_date(arr0, arr1)
        )
...
def test_time_slice_scalars(date_expr, func_args, answer, memory_leak_check):
    ...
    def impl(date_expr, slice_length):
        return bodo.libs.bodosql_array_kernels.time_slice(
...
def test_time_slice_columns(
    ...
    def impl(date_expr, slice_length):
        return pd.Series(
            bodo.libs.bodosql_array_kernels.time_slice(
