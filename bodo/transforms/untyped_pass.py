...
def remove_hiframes(rhs, lives, call_list):
    ...
    if (
        len(call_list) == 1
        and isinstance(call_list[0], CPUDispatcher)
    ...
    if (
        len(call_list) == 4
        and call_list[1:] == ["int_arr_ext", "libs", bodo]
        and call_list[0]
        in ["get_int_arr_data", "get_int_arr_bitmap", "init_integer_array"]
    ):
        return True
    if (
        len(call_list) == 4
    ...
    if (
        len(call_list) == 4
        and call_list[1:] == ["series_kernels", "hiframes", bodo]
        and call_list[0] in ["_sum_handle_nan", "_mean_handle_nan", "_var_handle_nan"]
    ):
        return True
        ...
        if (
            ...
            if is_call(arg_def) and guard(find_callname, self.func_ir, arg_def) == (
                ...
                return _compile_func_single_block(
                    lambda l: bodo.utils.typing.flatten_to_series(l), (in_data,), lhs
                )
