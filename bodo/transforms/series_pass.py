...
class SeriesPass(object):
    ...
    def _run_call_series(self, assign, lhs, rhs, series_var, func_name):
        ...
        if func_name == 'describe':
            ...
            impl = overload_func(*arg_typs, **kw_typs)
            stub = (
                lambda S, percentiles=None, include=None, exclude=None: None)
            return self._replace_func(impl, rhs.args,
                        pysig=numba.utils.pysignature(stub),
                        kws=dict(rhs.kws))
        ...
        if func_name == 'dropna':
            ...
            overload_func = getattr(bodo.hiframes.series_impl,
                'overload_series_' + func_name)
            impl = overload_func(*arg_typs, **kw_typs)
            stub = (lambda S, axis=0, inplace=False: None)
            return self._replace_func(impl, rhs.args,
                        pysig=numba.utils.pysignature(stub),
                        kws=dict(rhs.kws))
        ...
        if func_name in ('pct_change'):
            ...
            overload_func = getattr(bodo.hiframes.series_impl,
                'overload_series_' + func_name)
            impl = overload_func(*arg_typs, **kw_typs)
            stub = (lambda S, periods=1, fill_method='pad', limit=None,
                    freq=None: None)
            return self._replace_func(impl, rhs.args,
                        pysig=numba.utils.pysignature(stub),
                        kws=dict(rhs.kws))
