...
class DataFramePass(object):
    ...
    def _run_call_dataframe_apply(self, assign, lhs, rhs, df_var):
        ...
        f_ir = compile_to_numba_ir(
            f, {"numba": numba, "np": np, "Row": Row, "bodo": bodo, "_arr_typ": arr_typ}
        )
    ...
    def _run_call_df_dropna(self, assign, lhs, rhs):
        ...
        if not inplace:
            return nodes + compile_func_single_block(_init_df, out_cols, lhs, self)
        else:
            return self._set_df_inplace(_init_df, out_cols, df_var, lhs.loc, nodes)
