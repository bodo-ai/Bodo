...
void import_fs_module(Bodo_Fs::FsEnum fs_option, const std::string &file_type, 
    ...
    if (fs_option == Bodo_Fs::s3) {
        ...
        CHECK(f_mod, "importing bodo.io.hdfs_readerder module failed", file_type);
    }
}

void get_fs_reader_pyobject(Bodo_Fs::FsEnum fs_option, const std::string &file_type, 
    PyObject *f_mod, PyObject *&func_obj){
    if (fs_option == Bodo_Fs::s3) {
...
void get_get_fs_pyobject(Bodo_Fs::FsEnum fs_option, const std::string &file_type, 
    PyObject *f_mod, PyObject *&func_obj){
    if (fs_option == Bodo_Fs::s3) {
...
void open_file_outstream(Bodo_Fs::FsEnum fs_option, const std::string &file_type,
    ...
    if (fs_option == Bodo_Fs::posix){
        ...
        CHECK_ARROW(hdfs_fs->OpenWritable(fname, 
            false, &hdfs_out_stream), "Hdfs::OpenWritable", file_type);  
        *out_stream = hdfs_out_stream;                                        ;
     }
}

void open_file_appendstream(const std::string &file_type,
                         const std::string &fname, 
                         std::shared_ptr<::arrow::io::HadoopFileSystem> hdfs_fs,
                         std::shared_ptr<::arrow::io::OutputStream> *out_stream){
    std::shared_ptr<::arrow::io::HdfsOutputStream> hdfs_out_stream;
    CHECK_ARROW(hdfs_fs->OpenWritable(fname, 
        true, &hdfs_out_stream), "Hdfs::OpenWritable", file_type);  
    *out_stream = hdfs_out_stream;                                        ;
}
...
void open_outstream(Bodo_Fs::FsEnum fs_option, bool is_parallel, int myrank,
    ...
    if (fs_option == Bodo_Fs::posix) {
        ...
        if (is_parallel) {
            ...
            open_file_outstream(fs_option, file_type, fname, 
                NULL, NULL, out_stream);
        }
        return;
    } else if (fs_option == Bodo_Fs::s3) {
...
void parallel_in_order_write(Bodo_Fs::FsEnum fs_option, 
    const std::string &file_type, const std::string &fname,
    char* buff, int64_t count, int64_t elem_size,
    std::shared_ptr<arrow::fs::S3FileSystem> s3_fs,
    std::shared_ptr<::arrow::io::HadoopFileSystem> hdfs_fs){
    ...
    if (fs_option == Bodo_Fs::s3) {
        ...
        if (myrank != 0){
            ...
            MPI_Recv(&token, 1,  MPI_INT, 0, token_tag,
                MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            // send buff in chunks no bigger than INT_MAX
            while (sent_size < buff_size){
                if (buff_size - sent_size > INT_MAX) send_size = INT_MAX;
                else send_size = buff_size - sent_size;
                // send chunk size
                ierr = MPI_Send(&send_size, 1, MPI_INT, 0, size_tag, MPI_COMM_WORLD); 
                CHECK_MPI(ierr, err_class, err_string, &err_len, fname);
                // send chunk data;
                ierr = MPI_Send(buff + sent_size, send_size, MPI_CHAR, 0, data_tag, 
                    MPI_COMM_WORLD);
                CHECK_MPI(ierr, err_class, err_string, &err_len, fname);
                sent_size += send_size;
                if (sent_size == buff_size) complete = 1;
                // signal rank 0 whether the entire buff is sent
                ierr = MPI_Send(&complete, 1, MPI_INT, 0, complete_tag, MPI_COMM_WORLD); 
                CHECK_MPI(ierr, err_class, err_string, &err_len, fname);
            }
        }
        else{ 
            // 0 rank open outstream first
            open_file_outstream(Bodo_Fs::s3, "", fname, s3_fs, NULL, &out_stream);
                ...
                if (rank == 0) {
                    ...
                    while (complete == 0) {
                        ...
                        ierr = MPI_Recv(&recv_buff_size, 1,  MPI_INT, rank, 
                            size_tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                        CHECK_MPI(ierr, err_class, err_string, &err_len, fname);
                        // resize recv_buffer to fit incoming data
                        recv_buffer.resize(recv_buff_size);
                        // receive buffer data
                        ierr = MPI_Recv(&recv_buffer[0], recv_buff_size,  MPI_CHAR, 
                            rank, data_tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                        CHECK_MPI(ierr, err_class, err_string, &err_len, fname);
                        CHECK_ARROW(out_stream->Write(recv_buffer.data(), 
                            recv_buff_size), 
                            "arrow::io::S3OutputStream::Write", file_type);
                        // receive whether the entire buff is received
                        ierr = MPI_Recv(&complete, 1,  MPI_INT, rank, complete_tag, 
                            MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                        CHECK_MPI(ierr, err_class, err_string, &err_len, fname);
                    }
                }
                if (rank != num_ranks - 1) {
                    ierr = MPI_Send(&token, 1, MPI_INT, rank+1, 
            ...
            CHECK_ARROW(out_stream->Close(), 
                "arrow::io::S3OutputStream::Close", file_type);
        }
    } else if (fs_option == Bodo_Fs::hdfs) {
        ...
        if (myrank != 0){
            ...
            open_file_outstream(Bodo_Fs::hdfs, file_type, fname, 
                NULL, hdfs_fs, &out_stream);
        }
        ...
        CHECK_ARROW(out_stream->Close(), 
            "arrow::io::HdfsOutputStream::Close", file_type);

        // all but the last rank send message
        if (myrank != num_ranks -1){
            ierr = MPI_Send(&token, 1, MPI_INT, myrank + 1, token_tag, MPI_COMM_WORLD);
            CHECK_MPI(ierr, err_class, err_string, &err_len, fname);
        }
    }
    MPI_Barrier(MPI_COMM_WORLD);
