...
#define CHECK_ARROW(expr, msg/*, s3_fs_region*/)                          \
    if (!(expr.ok())) {                                                   \
        std::string err_msg =                                             \
            std::string("Error in arrow[gcsfs]: ") + msg                  \
            + " " + expr.ToString() + ".\n";                              \
...
std::shared_ptr<arrow::py::fs::PyFileSystem> get_gcs_fs() {
    ...
    if (!is_fs_initialized) {
        ...
        PyObject *GCSFileSystem = PyObject_GetAttrString(gcsfs_mod, "GCSFileSystem");
        Py_DECREF(gcsfs_mod);
        ...
        PyObject *handler = PyObject_CallMethod(pyarrow_fs_mod, "FSSpecHandler", "O", fs);
        ...
        pyfs = PyObject_CallMethod(bodo_pyfs_mod, "PyFileSystemBodo", "O", handler);
    ...
    std::shared_ptr<arrow::py::fs::PyFileSystem> pyfs_cpp =
           std::dynamic_pointer_cast<arrow::py::fs::PyFileSystem>(get_cpp_fs((c_PyFileSystemBodo*)pyfs));
    return pyfs_cpp;
}

static int finalize_gcs() {
    if (is_fs_initialized) {
        Py_DECREF(pyfs); // note that this will delete the underlying Arrow C++ objects as well
        pyfs = nullptr;
...
void gcs_open_file(const char *fname,
    ...
    std::shared_ptr<arrow::py::fs::PyFileSystem> fs = get_gcs_fs(/*bucket_region*/);
