...
inline void read_xe_row(uint8_t *&buf, uint8_t *&curr_arr, uint64_t tp_enum,
                        bool do_read, int &len);

extern "C" {

static PyObject *get_schema(PyObject *self, PyObject *args);
int64_t get_column_size_xenon(xe_connection_t xe_connection,
                              xe_dataset_t xe_dataset, uint64_t col_id);
...
void read_xenon_col_parallel(xe_connection_t xe_connection,
                             xe_dataset_t xe_dataset, uint64_t col_id,
                             uint8_t *arr, uint64_t *xe_typ_enums,
...
void read_xenon_col_str_parallel(xe_connection_t xe_connection,
                                 ...
                                 uint64_t *xe_typ_enums, uint64_t start,
                                 uint64_t count);
...
#define CHECK(expr, msg)               \
    ...
    CHECK(PyArg_ParseTuple(args, "ss", &address, &dset_name),
          "xenon dataset name expected");
    ...
    xe_dataset_t xe_dataset =
        xe_open(xe_connection, dset_name, 0, 0, XE_O_READONLY);
    ...
    if (xe_exists(xe_connection, dset_name, (const char **)&read_schema,
                  &fanout)) {
...
#define CHECK(expr, msg)               \
    ...
    // _type_to_xe_dtype_number = {'int8': 0, 'int16': 1, 'int32': 2, 'int64':
    // 3,
    ...
    for (uint64_t sid = 0; sid < status.fanout; sid++) {
        ...
        do {
            ...
            xe_get(xe_connection, xe_dataset, sid, read_buf, READ_BUF_SIZE,
                   &nrows);
...
#define CHECK(expr, msg)               \
    ...
    while (read_rows < count) {
        ...
        int64_t rows_to_read =
            std::min(count - read_rows, status.snrows - rows_to_skip);
        int64_t batch_ind = 0;

        do {
            uint8_t *buf = read_buf;
            err = xe_get(xe_connection, xe_dataset, strand_ind, read_buf,
                         READ_BUF_SIZE, &nrows);
            CHECK(!err, "Fail to read dataset");
            for (uint64_t r = batch_ind; r < batch_ind + nrows; r++) {
                bool do_read =
                    (r >= rows_to_skip) && (r < (rows_to_skip + rows_to_read));
                for (uint64_t c = 0; c < status.ncols; c++) {
                    uint64_t tp_enum = xe_typ_enums[c];
                    read_xe_row(buf, curr_arr, tp_enum,
                                (col_id == c) && do_read, len);
                }
            }
            batch_ind += nrows;
            if (batch_ind > (rows_to_skip + rows_to_read)) break;
        } while (nrows);
                ...
                for (uint64_t c = 0; c < status.ncols; c++) {
                    ...
                    if (col_id == c) {
                        *curr_offset = curr_len;
                        curr_offset++;
                        curr_len += len;
                        data_vec.insert(data_vec.end(), curr_arr - len,
                                        curr_arr);
                    }
                }
            }
...
xe_dataset_t c_xe_open(xe_connection_t xe_connect, char *dset) {
    ...
    xe_dataset_t xe_dataset =
        xe_open(xe_connect, dset_name, 0, 0, XE_O_READONLY);
