...
def _handle_read(assign, lhs, rhs, func_ir):
    ...
    if len(rhs.args) not in [1, 3]:
        raise ValueError(
            "read_xenon expects one or three argument but received {}".format(
                len(rhs.args)
            )
        )

    if len(rhs.args) == 1:
        out_nodes, col_names, col_types, xe_connect_var, xe_dset_var = gen_xe_init_from_uri(
            func_ir, rhs.args[0]
        )
    else:
    ...
    for i, cname in enumerate(col_names):
        ...
        out_nodes += get_column_read_nodes(
            c_type, cvar, xe_connect_var, xe_dset_var, i, schema_arr_var
        )
...
_xe_type_to_numba = {
    "BOOL": types.Array(types.boolean, 1, "C"),
...
def gen_close_xenon(connect_var, dset_t_var):
    ...
    f_block = compile_to_numba_ir(close_func, {"xe_close": xe_close}).blocks.popitem()[
        1
    ]
...
xe_open = types.ExternalFunction(
    "c_xe_open", xe_dset_type(xe_connect_type, types.voidptr)
)
xe_close = types.ExternalFunction(
    "c_xe_close", types.void(xe_connect_type, xe_dset_type)
)
...
def read_xenon_col(typingctx, connect_tp, dset_tp, col_id_tp, column_tp, schema_arr_tp):
    def codegen(context, builder, sig, args):
        ...
        res = builder.call(
            ...
            [
                ...
                args[1],
                args[2],
                builder.bitcast(arr_info.data, lir.IntType(8).as_pointer()),
                ctinfo.data,
            ],
        )
    ...
    return (
        signature(types.none, connect_tp, dset_tp, col_id_tp, column_tp, schema_arr_tp),
        codegen,
    )


@intrinsic
def read_xenon_col_parallel(
    typingctx,
    connect_tp,
    dset_tp,
    col_id_tp,
...
def read_xenon_str(typingctx, connect_tp, dset_tp, col_id_tp, size_tp, schema_arr_tp):
    ...
    return (
        signature(
            string_array_type, connect_tp, dset_tp, col_id_tp, size_tp, schema_arr_tp
        ),
        codegen,
    )
...
@intrinsic
def read_xenon_str_parallel(
    typingctx, connect_tp, dset_tp, col_id_tp, schema_arr_tp, start_tp, count_tp
):
    def codegen(context, builder, sig, args):
        typ = sig.return_type
        dtype = StringArrayPayloadType()
        ...
        fn = builder.module.get_or_insert_function(
            fnty, name="c_read_xenon_col_str_parallel"
        )
        res = builder.call(
            fn,
            [
