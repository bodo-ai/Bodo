...
std::shared_ptr<table_info> convertGPUToTable(GPU_DATA batch) {
    ...
    // We must create a cudf::column_metadata hierarchy matching the table structure.
    ...
    // We release the raw pointers from the unique_ptrs because Arrow's Import function
    // takes ownership of the C-structs and manages their lifecycle internally.
    struct ArrowSchema* raw_schema = unique_schema.release();
    struct ArrowDeviceArray* raw_array = unique_device_array.release();

    // ImportDeviceRecordBatch is the zero-copy bridge from the C-structs to C++ objects.
    // (The data is already on CPU from step 3, so this import is cheap).
    arrow::Result<std::shared_ptr<arrow::RecordBatch>> maybe_batch = arrow::ImportDeviceRecordBatch(raw_array, raw_schema);
    ...
    arrow::Result<std::shared_ptr<arrow::Table>> maybe_table = arrow::Table::FromRecordBatches({arrow_batch});
    if (!maybe_table.ok()) {
...
GPU_DATA convertTableToGPU(std::shared_ptr<table_info> batch) {
    ...
    // Arrow tables can have fragmented columns (chunks). libcudf expects contiguous memory.
    // This merges all chunks into a single RecordBatch.
    ...
    std::shared_ptr<arrow::Table> combined_table = combined_table_result.ValueOrDie();
    
    // Read the single batch
    arrow::TableBatchReader reader(*combined_table);
    arrow::Result<std::shared_ptr<arrow::RecordBatch>> batch_result = reader.Next();
    ...
    arrow::Status status = arrow::ExportRecordBatch(*arrow_batch, &arrow_array, &arrow_schema);
    if (!status.ok()) {
    ...
    // (ArrowDeviceArray contains an 'array' member which is the ArrowArray struct)
    ...
    // Move Data to GPU
    // from_arrow_host parses the structs, allocates GPU memory, and performs the copy.
    std::unique_ptr<cudf::table> result = cudf::from_arrow_host(&arrow_schema, &device_array);
