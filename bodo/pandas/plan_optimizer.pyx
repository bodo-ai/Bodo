...
import pandas as pd
from utils import  BodoLibNotImplementedError
...
def str_to_expr_type(val):
    if val is operator.eq:
        ...
        raise BodoLibNotImplementedError("Unhandled case in str_to_expr_type")
...
def join_type_to_string(CJoinType join_type):
    ...
    if join_type == CJoinType.INVALID:
        ...
        raise BodoLibNotImplementedError("Unknown Join Type")
...
cdef unique_ptr[CExpression] make_expr(val):
    ...
    if isinstance(val, int):
        ...
        if val.resolution.nanoseconds == 1:
            ...
            raise BodoLibNotImplementedError("Only support ns timestamp resolution currently, not " + str(val.resolution))
    else:
        raise BodoLibNotImplementedError("Unknown expr type in make_expr " + str(type(val)))
