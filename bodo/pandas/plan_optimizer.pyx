...
cdef extern from "_plan.h" nogil:
    ...
    cdef unique_ptr[CExpression] make_agg_expr(unique_ptr[CLogicalOperator] source, object field, c_string function_name, vector[int] input_column_indices, c_bool as_index, c_bool dropna) except +
...
cdef class AggregateExpression(Expression):
    ...
    def __cinit__(self, object out_schema, LogicalOperator source, str function_name, vector[int] input_column_indices, c_bool as_index, c_bool dropna):
        self.out_schema = out_schema
        self.function_name = function_name
        self.c_expression = make_agg_expr(source.c_logical_operator, out_schema[0], function_name.encode(), input_column_indices, as_index, dropna)
