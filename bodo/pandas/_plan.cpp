...
std::unique_ptr<duckdb::Expression> make_substring_expr(
    ...
    // Convert std::unique_ptr to duckdb::unique_ptr.
    auto lhs_duck = to_duckdb(lhs);
    auto start_duck = to_duckdb(starting_element);
    auto len_duck = to_duckdb(length);
    std::shared_ptr<arrow::Schema> out_schema = unwrap_schema(out_schema_py);
    ...
    // Build children: [lhs, start, length] with no additional checks or
    // clamping.
    duckdb::vector<duckdb::unique_ptr<duckdb::Expression>> children;
    children.emplace_back(std::move(lhs_duck));
    children.emplace_back(std::move(start_duck));
    children.emplace_back(std::move(len_duck));

    // Lookup substr (fallback to substring)
    duckdb::EntryLookupInfo function_lookup(
        duckdb::CatalogType::SCALAR_FUNCTION_ENTRY, "substr", error_context);
    duckdb::optional_ptr<duckdb::CatalogEntry> entry = binder->GetCatalogEntry(
        "system", "", function_lookup, duckdb::OnEntryNotFound::RETURN_NULL);

    if (!entry) {
        duckdb::EntryLookupInfo alt_lookup(
            duckdb::CatalogType::SCALAR_FUNCTION_ENTRY, "substring",
            error_context);
        entry = binder->GetCatalogEntry("system", "", alt_lookup,
        ...
        if (!entry) {
            ...
            throw std::runtime_error(
                "make_substring_expr GetCatalogEntry failed for "
                "'substr'/'substring'");
        }
    }
    ...
    if (!result) {
        ...
        throw std::runtime_error(
            "make_substring_expr BindScalarFunction failed");
    }
    ...
    if (result->GetExpressionType() != duckdb::ExpressionType::BOUND_FUNCTION) {
        ...
        throw std::runtime_error(
            "make_substring_expr BindScalarFunction did not return a "
            "BOUND_FUNCTION");
    }
    ...
    // Override return type to match Arrow schema (string)
    result->return_type = arrowTypeToDuckDB(out_schema->field(0)->type());
    return result;
}
