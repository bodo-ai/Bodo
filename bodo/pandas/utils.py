...
def run_func_on_table(cpp_table, result_type, in_args):
    ...
    cpp_to_py_start = time.monotonic_ns()
    if is_series:
    ...
    cpp_to_py = time.monotonic_ns() - cpp_to_py_start

    udf_time_start = time.monotonic_ns()
    if isinstance(func, str) and is_attr:
        ...
        out = func(input, *args, **kwargs)
    udf_time = time.monotonic_ns() - udf_time_start
    ...
    py_to_cpp_start = time.monotonic_ns()
    if is_series:
    ...
    py_to_cpp = time.monotonic_ns() - py_to_cpp_start
