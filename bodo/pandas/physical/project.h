    ...
    explicit PhysicalProjection(
        ...
        for (const auto& expr : this->exprs) {
            physical_exprs.emplace_back(
                buildPhysicalExprTree(expr, col_ref_map));
        ...
        for (auto& phys_expr : this->physical_exprs) {
            ...
            if (!res_as_array) {
                /*
                   ...
                   duckdb::ExpressionType::VALUE_CONSTANT) { auto& const_expr =
                                    expr->Cast<duckdb::BoundConstantExpression>();
                                size_t nrows = input_batch->nrows();

                                // Create an Arrow array filled with the
                   constant value std::shared_ptr<arrow::Array> arr =
                   std::visit( [nrows](const auto&& value) { return
                   ScalarToArrowArray(value, nrows);
                                    },
                                    extractValue(const_expr.value));

                                out_cols.emplace_back(
                                    arrow_array_to_bodo(arr,
