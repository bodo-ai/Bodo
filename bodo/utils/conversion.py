...
def overload_coerce_to_ndarray(
    ...
    if isinstance(data, bodo.libs.int_arr_ext.IntegerArrayType) and is_overload_none(
        ...
        return (
            lambda data, error_on_nonarray=True, use_nullable_array=None, scalar_to_arr_len=None: (
    ...
    if isinstance(data, bodo.libs.float_arr_ext.FloatingArrayType) and is_overload_none(
        ...
        return (
            lambda data, error_on_nonarray=True, use_nullable_array=None, scalar_to_arr_len=None: (
                bodo.libs.float_arr_ext.get_float_arr_data(data)
            )
        )  # pragma: no cover
    ...
    if isinstance(data, types.Array):
        if not is_overload_none(use_nullable_array) and (
            ...
            return (
                lambda data, error_on_nonarray=True, use_nullable_array=None, scalar_to_arr_len=None: (
                    bodo.utils.conversion.np_to_nullable_array(data)
                )
            )  # pragma: no cover
    ...
    if isinstance(data, SeriesType):
        return (
            lambda data, error_on_nonarray=True, use_nullable_array=None, scalar_to_arr_len=None: (
                bodo.hiframes.pd_series_ext.get_series_data(data)
            )
        )  # pragma: no cover
...
def overload_coerce_scalar_to_array(scalar, length, arr_type, dict_encode=True):
    ...
    if _arr_typ == bodo.types.null_array_type:
        return lambda scalar, length, arr_type, dict_encode=True: (
            bodo.libs.null_arr_ext.init_null_array(length)
        )  # pragma: no cover
...
def overload_coerce_to_array(
    ...
    if isinstance(data, SeriesType):
        ...
        return (
            lambda data, error_on_nonarray=True, use_nullable_array=None, scalar_to_arr_len=None, dict_encode=True: (
                bodo.hiframes.pd_series_ext.get_series_data(data)
            )
        )  # pragma: no cover
    ...
    if isinstance(data, types.List) and data.dtype in (
        ...
        return (
            lambda data, error_on_nonarray=True, use_nullable_array=None, scalar_to_arr_len=None, dict_encode=True: (
                bodo.libs.str_arr_ext.str_arr_from_sequence(data)
            )
        )  # pragma: no cover
    ...
    if isinstance(data, types.BaseTuple) and data.count == 0:
        return (
            lambda data, error_on_nonarray=True, use_nullable_array=None, scalar_to_arr_len=None, dict_encode=True: (
                bodo.libs.str_arr_ext.empty_str_arr(data)
            )
        )  # pragma: no cover
...
def overload_fix_arr_dtype(
    ...
    if is_overload_none(new_dtype) or is_object:
        if do_copy:
            return (
                lambda data, new_dtype, copy=None, nan_to_str=False, from_series=False: (
                    data.copy()
                )
            )  # pragma: no cover
        return lambda data, new_dtype, copy=None, nan_to_str=False, from_series=False: (
            data
        )  # pragma: no cover
    ...
    if isinstance(data, bodo.types.ArrayItemArrayType):
        ...
        call_id = next_label()
        func_text = "def impl(data, new_dtype, copy=None, nan_to_str=False, from_series=False):\n"
        func_text += "  inner_data_arrs = bodo.libs.struct_arr_ext.get_data(data)\n"
        for i in range(n_fields):
    ...
    if isinstance(data, NullableTupleType):
        ...
        for typ in scalar_types:
            ...
            elif typ == bodo.types.timedelta64ns and nb_dtype not in (
                types.unicode_type,
                types.int64,
                types.uint64,
                bodo.types.timedelta64ns,
            ):
                raise BodoError(f"invalid type conversion from {typ} to {nb_dtype}.")
...
def overload_index_from_array(data, name=None):
    ...
    if isinstance(data, bodo.hiframes.pd_categorical_ext.CategoricalArrayType):
        return lambda data, name=None: (
            bodo.hiframes.pd_index_ext.init_categorical_index(data, name)
        )  # pragma: no cover
...
def overload_box_if_dt64(val):
    ...
    if val == types.NPDatetime("ns"):
        return lambda val: (
            bodo.hiframes.pd_timestamp_ext.convert_datetime64_to_timestamp(val)
        )  # pragma: no cover
