...
def pyarray_getitem(builder, context, arr_obj, ind):
    ...
    arr_get_fnty = lir.FunctionType(
        lir.IntType(8).as_pointer(), [pyobj, py_ssize_t]
    )
    arr_get_fn = builder.module.get_or_insert_function(arr_get_fnty, name="array_getptr1")
    arr_getitem_fnty = lir.FunctionType(
        pyobj, [pyobj, lir.IntType(8).as_pointer()]
    )
    arr_getitem_fn = builder.module.get_or_insert_function(arr_getitem_fnty, name="array_getitem")
...
def is_na_value(builder, context, val, C_NA):
    ...
    """
    pyobj = context.get_argument_type(types.pyobject)
    arr_isna_fnty = lir.FunctionType(
        lir.IntType(32), [pyobj, pyobj]
    )
    arr_isna_fn = builder.module.get_or_insert_function(arr_isna_fnty, name="is_na_value")
