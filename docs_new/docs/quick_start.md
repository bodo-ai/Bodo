...
This section provides a quick start guide to Bodo and explains its
...
See the section on [bodo parallelism basics][basics] for more
...
[comment]: <> (Autorefs to '[install], [basics], [caching], [pandas], [ipyparallelsetup] will format once those sections are added)

[todo]: <> (modify/remove comment above as the '[install], [basics], [caching], [pandas], [ipyparallelsetup] sections are added)