...
- **IAM role**
  ...
  1. Select the *Roles* tab in the sidebar, and click `Create Role`.

  1. In **Select type of trusted entity**, select `Another AWS Account`.

  1. Enter the Bodo Platform Account ID `481633624848` in the
     **Account ID** field.

  1. Check the `Require external ID` option.
  ...
  1. In the **Role name** field, enter a role name, e.g.
     `BodoPlatformUser`.

     ![Create Role Form Review](../create_role_screenshots/create_iam_role_manual_review_step.png#center)

  1. Click `Create Role`. You will be taken back to the list of IAM Roles
     in your account.

  1. In the list of IAM Roles, click on the role you just created.

  1. Under Permissions tab, Click on `Add Permissions` and select `Create inline policy` from the dropdown.
  ...
  1. Bodo Cloud Platform requires a specific set of AWS permissions which
  ...
  1. In the *Name* field, add a policy name, e.g.
     `Bodo-Platform-User-Policy`. Click on `Create policy`.
     You will be taken back to the Role Summary.

  1. From the role summary, copy the `Role ARN`. This is the value that
...
1. Enter the Role ARN in the **Role ARN** field.
1. Click on `Create`.
...
1. Click on the `Select metric` which would bring you to a search bar that allows you
...
1. Set a reasonable number for the threshold for an alarm to go off based on your usage expectations.
...
1. You will now be asked to select the Simple Notification Service (SNS) Topic for this alarm.
...
1. You will now be required to fill out the details of the alarm itself, fill the fields with the
   appropriate details and click `Next` at the bottom of the page.

1. Finally, preview your alarm before you click on `Create Alarm` at the bottom of the page to create the Alarm.
