...
This release, we are excited to add support for more GPU operators including sort, Iceberg I/O, and string functions in [GPU execution mode][df_gpu], available through the CUDA-enabled variant of our conda package, in addition to several bug fixes and UX improvements.
...
In addition to these features, we also made the following improvements:
...
- Fixed correctness issues in groupby nunique().
...
- Added support for lists of functions in SeriesGroupby & DataFrameGroupby aggregate() function dicts (i.e. df.groupby(...).agg({“col1”: [“mean”, “count”], “col2”: [“sum”, “count”]})`).

### 🐛 Bug Fixes

- Fixed null and NA handling to match Pandas.

- Fixed bug in parallel join termination condition.
...
- Improvements to the BodoSQL C++ backend, enabling more types of queries to run without falling back to JIT compilation.
