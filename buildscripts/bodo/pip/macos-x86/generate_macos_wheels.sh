...
do
    ...
    if python -c 'import sys; sys.exit(sys.version_info[:2] >= (3, 10))'; then
        ...
        python -m pip install Cython "numpy==1.18.*" wheel pyarrow==11.0.0 mpi4py_mpich==3.1.2
    else
        # exit code 1 when version >= 3.10
        python -m pip install Cython "numpy==1.21.*" wheel pyarrow==11.0.0 mpi4py_mpich==3.1.2
    fi
    PYARROW_PATH=`python -c "import pyarrow; print('/'.join(pyarrow.__file__.split('/')[:-1]))"`
    ln -s $PYARROW_PATH/libarrow.1100.dylib        $PYARROW_PATH/libarrow.so
    ln -s $PYARROW_PATH/libparquet.1100.dylib      $PYARROW_PATH/libparquet.so
