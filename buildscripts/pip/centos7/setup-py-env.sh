...
for PYBIN in /opt/python/cp*/bin; do
    ...
    if "${PYBIN}/python" -c 'import sys; sys.exit(sys.version_info[:2] >= (3, 10))'; then
        ...
        "${PYBIN}/pip" install astor twine Cython numpy==1.18.* wheel pyarrow==8.0.0
    else
        # exit code 1 when version >= 3.10
        "${PYBIN}/pip" install astor twine Cython numpy==1.21.* wheel pyarrow==8.0.0
    fi
    PYARROW_PATH=`"${PYBIN}/python" -c "import pyarrow; print('/'.join(pyarrow.__file__.split('/')[:-1]))"`
    ln -s $PYARROW_PATH/libarrow.so.800        $PYARROW_PATH/libarrow.so
    ln -s $PYARROW_PATH/libarrow_python.so.800 $PYARROW_PATH/libarrow_python.so
    ln -s $PYARROW_PATH/libparquet.so.800      $PYARROW_PATH/libparquet.so
done
