...
def create_cluster_helper(client, bodo_version, img_id, cluster_name, instance_type, description):
    """
    ...
    while True:
        ...
        cluster_details = client.cluster.get(cluster_uuid)
        if cluster_details.status == ClusterStatus.RUNNING or cluster_details.status == ClusterStatus.FAILED or tries == 50:
            break
        else:
            tries += 1
...
def create_job_helper(job_name, script, input_var, client, instance_role_uuid, workspace_path=None,
                        img_id=None, instance_type=None, workers=2, cluster_uuid=None, branch=""):
    """
    ...
    while True:
        ...
        if job_details.status == JobStatus.FINISHED or job_details.status == JobStatus.FAILED or tries == 50:
            break
        else:
            tries += 1
...
def create_instance_role_def():
    ...
    role_definition = CreateRoleDefinition(
        ...
        data=InstanceRole(role_arn="arn:aws:iam::427443013497:role/e2e-image-test-role")
    )
