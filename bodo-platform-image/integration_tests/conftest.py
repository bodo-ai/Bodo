...
from integration_tests.common import create_cluster_helper, remove_cluster, create_instance_role_def
...
def bodo_client():
    ...
    try:
        ...
        return get_bodo_client(keys, api_url=os.environ['BACKEND_API_URL'], auth_url=os.environ['BACKEND_AUTH_URL'])
    except Exception as e:
        logging.error(f"Exception while initializing bodo-client: {str(e)}")
        raise e

@pytest.fixture(scope='session')
def create_cluster(bodo_client, img_id, bodo_version, default_cluster_name, default_cluster_type, cluster_description):
    """
    Calls the helper function to create a cluster based on the passed in image_id and bodo_version and other metadata
    """
    cluster_metadata = create_cluster_helper(bodo_client, bodo_version, img_id, default_cluster_name, default_cluster_type, cluster_description)
    try:
...
def create_instance_role(bodo_client):
    ...
    if os.environ['CLOUD_PROVIDER'] == 'AWS':
        ...
        try:
            ...
            bodo_client.instance_role.remove(create_response.uuid) # cleanup role created
    else:
        yield None
