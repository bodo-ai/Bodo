...
def is_older_than_x_days(group_details, x):
    ...
    timedelta_since_creation: timedelta = datetime.now(
        timezone.utc) - creation_date
...
def delete_resource_groups(resource_groups_to_remove):
    ...
    for group in resource_groups_to_remove:
        ...
        delete_async_operation = client.resource_groups.begin_delete(
            group, force_deletion_types='Microsoft.Compute/virtualMachines,Microsoft.Compute/virtualMachineScaleSets')
...
if __name__ == "__main__":
    ...
    print(
        f"Writing information about resource groups to remove to {args.output}")
